...
// File generated at Thu 12 Oct 2017 14:02:53
...
SingletDMZ3_spectrum_plotter::SingletDMZ3_spectrum_plotter(const SingletDMZ3_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void SingletDMZ3_spectrum_plotter::extract_spectrum(const SingletDMZ3_mass_eigenstates& model)
{
   spectrum.clear();
...
void SingletDMZ3_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("SingletDMZ3_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("SingletDMZ3_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("SingletDMZ3_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void SingletDMZ3_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("SingletDMZ3_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const SingletDMZ3_mass_eigenstates& model,
   ...
   const SingletDMZ3_observables* observables)
{
   ...
   append(names, SingletDMZ3_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, SingletDMZ3_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, SingletDMZ3_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, SingletDMZ3_parameter_getter().get_DRbar_mass_names());
   append(names, SingletDMZ3_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, SingletDMZ3_parameter_getter().get_pole_mass_names());
   append(names, SingletDMZ3_parameter_getter().get_pole_mixing_names());
   ...
   if (SingletDMZ3_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, SingletDMZ3_observables::get_names());
...
SingletDMZ3_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, SingletDMZ3_observables* observables)
{
   using utilities::append;

   SingletDMZ3_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = SingletDMZ3_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = SingletDMZ3_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = SingletDMZ3_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = SingletDMZ3_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? SingletDMZ3_observables::NUMBER_OF_OBSERVABLES : 0);
