...
/**
 * @file SingletDMZ3_a_muon.cpp
 *
 * This file was generated at Tue 20 Feb 2018 16:02:41 with FlexibleSUSY
...
#include "SingletDMZ3_cxx_diagrams.hpp"
...
double get_MSUSY(const SingletDMZ3_mass_eigenstates& model)
{
   return 0.;

}

void run_to_MSUSY(SingletDMZ3_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("SingletDMZ3_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("SingletDMZ3_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("SingletDMZ3_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("SingletDMZ3_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(SingletDMZ3_mass_eigenstates& model)
{
   VERBOSE_MSG("SingletDMZ3_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const SingletDMZ3_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("SingletDMZ3_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double SingletDMZ3_a_muon::calculate_a_muon(const SingletDMZ3_mass_eigenstates& model_)
{
   SingletDMZ3_mass_eigenstates model(model_);

   VERBOSE_MSG("SingletDMZ3_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SingletDMZ3_a_muon:" << e.what());
...
double SingletDMZ3_a_muon::calculate_a_muon_uncertainty(const SingletDMZ3_mass_eigenstates& model_)
{
   SingletDMZ3_mass_eigenstates model(model_);

   VERBOSE_MSG("SingletDMZ3_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SingletDMZ3_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
