...
// File generated at Tue 20 Feb 2018 16:02:46
...
namespace SingletDMZ3_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSSingletDMZ3Message", 1);
...
class SingletDMZ3_spectrum {
public:
   virtual ~SingletDMZ3_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(SingletDMZ3_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const SingletDMZ3_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const SingletDMZ3_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class SingletDMZ3_spectrum_impl : public SingletDMZ3_spectrum
{
public:
   virtual ~SingletDMZ3_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(SingletDMZ3_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const SingletDMZ3_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const SingletDMZ3_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<SingletDMZ3_slha<SingletDMZ3<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   SingletDMZ3_scales scales{};              ///< scale information
   SingletDMZ3_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const SingletDMZ3_input_parameters& input_) { input = input_; }
...
private:
   SingletDMZ3_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<SingletDMZ3_spectrum> spectrum{nullptr};  ///< spectrum information

   SingletDMZ3_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, SingletDMZ3_info::particle_names[i].c_str());
      }
   }
}
...
SingletDMZ3_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const SingletDMZ3_slha<Model>& model, MLINK link)
{
   MLPutRule(link, SingletDMZ3_info::model_name);
...
template <typename Solver_type>
void SingletDMZ3_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void SingletDMZ3_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const SingletDMZ3_input_parameters& input
)
{
   SingletDMZ3_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void SingletDMZ3_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void SingletDMZ3_spectrum_impl<Solver_type>::fill_slha_io(SingletDMZ3_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   SingletDMZ3_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSSingletDMZ3CalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSSingletDMZ3CalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new SingletDMZ3_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSSingletDMZ3GetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3GetSettings"))
...
DLLEXPORT int FSSingletDMZ3GetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3GetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMZ3GetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3GetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMZ3OpenHandle(
...
DLLEXPORT int FSSingletDMZ3CloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;
...
DLLEXPORT int FSSingletDMZ3Set(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::SingletDMZ3_librarylink;
...
DLLEXPORT int FSSingletDMZ3GetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3GetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMZ3ToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3ToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMZ3GetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3GetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMZ3CalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3CalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMZ3CalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDMZ3_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMZ3CalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSSingletDMZ3CalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSSingletDMZ3CalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSSingletDMZ3CalculateObservables", "error", e.what());
      put_error_output(link);
   }
