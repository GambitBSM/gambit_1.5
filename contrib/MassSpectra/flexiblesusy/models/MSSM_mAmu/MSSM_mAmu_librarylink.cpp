...
// File generated at Thu 12 Oct 2017 13:57:57
...
#include "MSSM_mAmu_physical.hpp"
#include "MSSM_mAmu_slha_io.hpp"
#include "MSSM_mAmu_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "MSSM_mAmu_two_scale_model.hpp"
#include "MSSM_mAmu_two_scale_spectrum_generator.hpp"
#endif
...
namespace MSSM_mAmu_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSM_mAmuMessage", 1);
...
class MSSM_mAmu_spectrum {
public:
   virtual ~MSSM_mAmu_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSM_mAmu_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSM_mAmu_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSM_mAmu_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSM_mAmu_spectrum_impl : public MSSM_mAmu_spectrum
{
public:
   virtual ~MSSM_mAmu_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSM_mAmu_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSM_mAmu_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSM_mAmu_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSM_mAmu_slha<MSSM_mAmu<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSM_mAmu_scales scales{};              ///< scale information
   MSSM_mAmu_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSM_mAmu_input_parameters& input_) { input = input_; }
...
private:
   MSSM_mAmu_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSM_mAmu_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSM_mAmu_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSM_mAmu_info::particle_names[i].c_str());
      }
   }
}
...
MSSM_mAmu_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSM_mAmu_slha_io slha_io;
...
void put_spectrum(const MSSM_mAmu_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSM_mAmu_info::model_name);
...
template <typename Solver_type>
void MSSM_mAmu_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSM_mAmu_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSM_mAmu_input_parameters& input
)
{
   MSSM_mAmu_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSM_mAmu_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSM_mAmu_spectrum_impl<Solver_type>::fill_slha_io(MSSM_mAmu_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSM_mAmu_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSM_mAmuCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSM_mAmuCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSM_mAmu_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSM_mAmuGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuGetSettings"))
...
DLLEXPORT int FSMSSM_mAmuGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSM_mAmuGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSM_mAmuOpenHandle(
...
DLLEXPORT int FSMSSM_mAmuCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;
...
DLLEXPORT int FSMSSM_mAmuGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSM_mAmuToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSM_mAmuGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSM_mAmuCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSM_mAmuCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSM_mAmuCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSM_mAmuCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSM_mAmuCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSM_mAmuCalculateObservables", "error", e.what());
      put_error_output(link);
   }
