...
double MSSM_mAmu_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSM_mAmu_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSM_mAmu_input_parameters& input)
{
   ...
   MSSM_mAmu_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSM_mAmu_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSM_mAmu_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSM_mAmu_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSM_mAmu_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSM_mAmu_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSM_mAmu_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSM_mAmu_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSM_mAmu_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSM_mAmu_spectrum_generator<Two_scale>::calculate_spectrum()
{
