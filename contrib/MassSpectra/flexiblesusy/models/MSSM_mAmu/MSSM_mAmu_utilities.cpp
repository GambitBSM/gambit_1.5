...
MSSM_mAmu_spectrum_plotter::MSSM_mAmu_spectrum_plotter(const MSSM_mAmu_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void MSSM_mAmu_spectrum_plotter::extract_spectrum(const MSSM_mAmu_mass_eigenstates& model)
{
   spectrum.clear();
...
void MSSM_mAmu_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MSSM_mAmu_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MSSM_mAmu_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MSSM_mAmu_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MSSM_mAmu_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MSSM_mAmu_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const MSSM_mAmu_mass_eigenstates& model,
   ...
   const MSSM_mAmu_observables* observables)
{
   ...
   append(names, MSSM_mAmu_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MSSM_mAmu_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, MSSM_mAmu_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, MSSM_mAmu_parameter_getter().get_DRbar_mass_names());
   append(names, MSSM_mAmu_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, MSSM_mAmu_parameter_getter().get_pole_mass_names());
   append(names, MSSM_mAmu_parameter_getter().get_pole_mixing_names());
   ...
   if (MSSM_mAmu_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MSSM_mAmu_observables::get_names());
 ...
 */
MSSM_mAmu_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, MSSM_mAmu_observables* observables)
{
   using utilities::append;

   MSSM_mAmu_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = MSSM_mAmu_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = MSSM_mAmu_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = MSSM_mAmu_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = MSSM_mAmu_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? MSSM_mAmu_observables::NUMBER_OF_OBSERVABLES : 0);
