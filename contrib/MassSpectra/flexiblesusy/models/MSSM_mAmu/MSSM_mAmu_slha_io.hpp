...
// File generated at Thu 12 Oct 2017 13:57:34

#ifndef MSSM_mAmu_SLHA_IO_H
#define MSSM_mAmu_SLHA_IO_H
...
public:
   ...
   void fill(MSSM_mAmu_input_parameters&) const;
   void fill(MSSM_mAmu_mass_eigenstates&) const;
   template <class Model> void fill(MSSM_mAmu_slha<Model>&) const;
   ...
   template <class Model> void set_extra(const MSSM_mAmu_slha<Model>&, const MSSM_mAmu_scales&, const MSSM_mAmu_observables&);
   void set_input(const MSSM_mAmu_input_parameters&);
   ...
   template <class Model> void set_spectrum(const MSSM_mAmu_slha<Model>&);
   template <class T> void set_spectrum(const MSSM_mAmu<T>&);
   ...
   static void fill_minpar_tuple(MSSM_mAmu_input_parameters&, int, double);
   static void fill_extpar_tuple(MSSM_mAmu_input_parameters&, int, double);
   static void fill_imminpar_tuple(MSSM_mAmu_input_parameters&, int, double);
   static void fill_imextpar_tuple(MSSM_mAmu_input_parameters&, int, double);

   template <class Model>
   static void fill_slhaea(SLHAea::Coll&, const MSSM_mAmu_slha<Model>&, const softsusy::QedQcd&, const MSSM_mAmu_scales&, const MSSM_mAmu_observables&);

   template <class Model>
   static SLHAea::Coll fill_slhaea(const MSSM_mAmu_slha<Model>&, const softsusy::QedQcd&, const MSSM_mAmu_scales&, const MSSM_mAmu_observables&);

private:
   SLHA_io slha_io; ///< SLHA io class
   bool print_imaginary_parts_of_majorana_mixings;

   void set_extpar(const MSSM_mAmu_input_parameters&);
   void set_imminpar(const MSSM_mAmu_input_parameters&);
   void set_imextpar(const MSSM_mAmu_input_parameters&);
   void set_minpar(const MSSM_mAmu_input_parameters&);
   void set_mass(const MSSM_mAmu_physical&, bool);
   void set_mixing_matrices(const MSSM_mAmu_physical&, bool);
   template <class Model> void set_model_parameters(const MSSM_mAmu_slha<Model>&);
   void set_ckm(const Eigen::Matrix<std::complex<double>,3,3>&, double);
   void set_pmns(const Eigen::Matrix<std::complex<double>,3,3>&, double);
   double read_scale() const;
   void fill_drbar_parameters(MSSM_mAmu_mass_eigenstates&) const;
   void fill_physical(MSSM_mAmu_physical&) const;
};
 ...
 */
template <class Model>
void MSSM_mAmu_slha_io::fill(MSSM_mAmu_slha<Model>& model) const
{
   fill(static_cast<MSSM_mAmu_mass_eigenstates&>(model));
   fill_physical(model.get_physical_slha());
}

template <class Model>
void MSSM_mAmu_slha_io::fill_slhaea(
   SLHAea::Coll& slhaea, const MSSM_mAmu_slha<Model>& model,
   const softsusy::QedQcd& qedqcd, const MSSM_mAmu_scales& scales,
   const MSSM_mAmu_observables& observables)
{
   MSSM_mAmu_slha_io slha_io;
   const MSSM_mAmu_input_parameters& input = model.get_input();
...
template <class Model>
SLHAea::Coll MSSM_mAmu_slha_io::fill_slhaea(
   const MSSM_mAmu_slha<Model>& model, const softsusy::QedQcd& qedqcd,
   const MSSM_mAmu_scales& scales, const MSSM_mAmu_observables& observables)
{
   SLHAea::Coll slhaea;
   MSSM_mAmu_slha_io::fill_slhaea(slhaea, model, qedqcd, scales, observables);
 ...
 */
template <class Model>
void MSSM_mAmu_slha_io::set_model_parameters(const MSSM_mAmu_slha<Model>& model)
{
   {
 ...
 */
template <class Model>
void MSSM_mAmu_slha_io::set_extra(
   const MSSM_mAmu_slha<Model>& model, const MSSM_mAmu_scales& scales,
   const MSSM_mAmu_observables& observables)
{
   const MSSM_mAmu_physical physical(model.get_physical_slha());
 ...
 */
template <class... Ts>
void MSSM_mAmu_slha_io::set_spectrum(const std::tuple<Ts...>& models)
{
   Set_spectrum<MSSM_mAmu_slha_io> ss(this);
 ...
 */
template <class T>
void MSSM_mAmu_slha_io::set_spectrum(const MSSM_mAmu<T>& model)
{
   set_spectrum(MSSM_mAmu_slha<MSSM_mAmu<T> >(model));
}
 ...
 */
template <class Model>
void MSSM_mAmu_slha_io::set_spectrum(const MSSM_mAmu_slha<Model>& model)
{
   const MSSM_mAmu_physical physical(model.get_physical_slha());
