...
MSSM_mAmu_convergence_tester<Two_scale>::MSSM_mAmu_convergence_tester(
   MSSM_mAmu<Two_scale>* model, double accuracy_goal, const Scale_getter& sg)
   : Convergence_tester_DRbar<MSSM_mAmu<Two_scale> >(model, accuracy_goal, sg)
{
}

double MSSM_mAmu_convergence_tester<Two_scale>::max_rel_diff() const
{
   const MSSM_mAmu<Two_scale>& ol = get_last_iteration_model();
   const MSSM_mAmu<Two_scale>& ne = get_current_iteration_model();
