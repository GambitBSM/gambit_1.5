...
{
   std::cout <<
      "Usage: scan_MSSMatMSUSYEFTHiggs_mAmu.x [options]\n"
      "Options:\n"
      "  --M1Input=<value>\n"
      "  --M2Input=<value>\n"
      "  --M3Input=<value>\n"
...
struct MSSMatMSUSYEFTHiggs_mAmu_scan_result {
   Spectrum_generator_problems problems;
   double higgs{0.};
};

template <class solver_type>
MSSMatMSUSYEFTHiggs_mAmu_scan_result run_parameter_point(const softsusy::QedQcd& qedqcd,
   MSSMatMSUSYEFTHiggs_mAmu_input_parameters& input)
{
   ...
   MSSMatMSUSYEFTHiggs_mAmu_scan_result result;
...
void scan(int solver_type, MSSMatMSUSYEFTHiggs_mAmu_input_parameters& input,
          const std::vector<double>& range)
{
   softsusy::QedQcd qedqcd;

   for (const auto p: range) {
      INPUTPARAMETER(M1Input) = p;
