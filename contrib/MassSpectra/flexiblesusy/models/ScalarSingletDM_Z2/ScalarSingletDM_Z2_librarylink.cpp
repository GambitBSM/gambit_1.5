...
// File generated at Sat 26 May 2018 14:35:29

#include "config.h"

#include "ScalarSingletDM_Z2_info.hpp"
#include "ScalarSingletDM_Z2_input_parameters.hpp"
#include "ScalarSingletDM_Z2_observables.hpp"
#include "ScalarSingletDM_Z2_physical.hpp"
#include "ScalarSingletDM_Z2_slha_io.hpp"
#include "ScalarSingletDM_Z2_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "ScalarSingletDM_Z2_two_scale_model.hpp"
#include "ScalarSingletDM_Z2_two_scale_spectrum_generator.hpp"
#endif
...
namespace ScalarSingletDM_Z2_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSScalarSingletDM_Z2Message", 1);
...
class ScalarSingletDM_Z2_spectrum {
public:
   virtual ~ScalarSingletDM_Z2_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(ScalarSingletDM_Z2_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const ScalarSingletDM_Z2_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const ScalarSingletDM_Z2_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class ScalarSingletDM_Z2_spectrum_impl : public ScalarSingletDM_Z2_spectrum
{
public:
   virtual ~ScalarSingletDM_Z2_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(ScalarSingletDM_Z2_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const ScalarSingletDM_Z2_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const ScalarSingletDM_Z2_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<ScalarSingletDM_Z2_slha<ScalarSingletDM_Z2<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   ScalarSingletDM_Z2_scales scales{};              ///< scale information
   ScalarSingletDM_Z2_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const ScalarSingletDM_Z2_input_parameters& input_) { input = input_; }
...
private:
   ScalarSingletDM_Z2_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<ScalarSingletDM_Z2_spectrum> spectrum{nullptr};  ///< spectrum information

   ScalarSingletDM_Z2_slha_io get_slha_io() const;
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(muSInput), "muSInput");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, ScalarSingletDM_Z2_info::particle_names[i].c_str());
      }
   }
}
...
ScalarSingletDM_Z2_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   ScalarSingletDM_Z2_slha_io slha_io;
...
void put_spectrum(const ScalarSingletDM_Z2_slha<Model>& model, MLINK link)
{
   MLPutRule(link, ScalarSingletDM_Z2_info::model_name);
   ...
   MLPutRuleTo(link, MODELPARAMETER(Yd), "Yd");
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(muS), "muS");
   MLPutRuleTo(link, MODELPARAMETER(muH), "muH");
   MLPutRuleTo(link, MODELPARAMETER(v), "v");
   MLPutRuleTo(link, MODELPARAMETER(scale), "SCALE");
...
template <typename Solver_type>
void ScalarSingletDM_Z2_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void ScalarSingletDM_Z2_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const ScalarSingletDM_Z2_input_parameters& input
)
{
   ScalarSingletDM_Z2_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void ScalarSingletDM_Z2_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void ScalarSingletDM_Z2_spectrum_impl<Solver_type>::fill_slha_io(ScalarSingletDM_Z2_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   ScalarSingletDM_Z2_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSScalarSingletDM_Z2CalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSScalarSingletDM_Z2CalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new ScalarSingletDM_Z2_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(muSInput) = pars[c++];
...
DLLEXPORT int FSScalarSingletDM_Z2GetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2GetSettings"))
...
DLLEXPORT int FSScalarSingletDM_Z2GetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2GetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z2GetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2GetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z2OpenHandle(
...
DLLEXPORT int FSScalarSingletDM_Z2CloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;
...
DLLEXPORT int FSScalarSingletDM_Z2Set(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;
...
DLLEXPORT int FSScalarSingletDM_Z2GetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2GetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z2ToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2ToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z2GetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2GetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z2CalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2CalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z2CalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z2_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z2CalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSScalarSingletDM_Z2CalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSScalarSingletDM_Z2CalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSScalarSingletDM_Z2CalculateObservables", "error", e.what());
      put_error_output(link);
   }
