...
// File generated at Thu 12 Oct 2017 13:53:44

#ifndef MSSMatMGUT_mAmu_SPECTRUM_GENERATOR_INTERFACE_H
#define MSSMatMGUT_mAmu_SPECTRUM_GENERATOR_INTERFACE_H

#include "MSSMatMGUT_mAmu_mass_eigenstates.hpp"
#include "MSSMatMGUT_mAmu_model.hpp"
...
template <class T>
class MSSMatMGUT_mAmu_spectrum_generator_interface {
public:
   virtual ~MSSMatMGUT_mAmu_spectrum_generator_interface() = default;

   std::tuple<MSSMatMGUT_mAmu<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSMatMGUT_mAmu_slha<MSSMatMGUT_mAmu<T>>> get_models_slha() const
   { return std::make_tuple(MSSMatMGUT_mAmu_slha<MSSMatMGUT_mAmu<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   MSSMatMGUT_mAmu<T> get_model() const
   { return model; }
   MSSMatMGUT_mAmu_slha<MSSMatMGUT_mAmu<T>> get_model_slha() const
   { return MSSMatMGUT_mAmu_slha<MSSMatMGUT_mAmu<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMatMGUT_mAmu_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMatMGUT_mAmu_spectrum.dat") const;

protected:
   MSSMatMGUT_mAmu<T> model;
   ...
   void translate_exception_to_problem(MSSMatMGUT_mAmu<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMatMGUT_mAmu_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMatMGUT_mAmu_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMatMGUT_mAmu_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMatMGUT_mAmu_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMatMGUT_mAmu_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMatMGUT_mAmu_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMatMGUT_mAmu_parameter_getter parameter_getter;
   Coupling_monitor<MSSMatMGUT_mAmu_mass_eigenstates, MSSMatMGUT_mAmu_parameter_getter>
 ...
 */
template <class T>
void MSSMatMGUT_mAmu_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMatMGUT_mAmu_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMatMGUT_mAmu_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMatMGUT_mAmu<T>& model)
{
   try {
      throw;
