...
#include "MSSMatMGUT_mAmu_utilities.hpp"
...
MSSMatMGUT_mAmu_spectrum_plotter::MSSMatMGUT_mAmu_spectrum_plotter(const MSSMatMGUT_mAmu_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void MSSMatMGUT_mAmu_spectrum_plotter::extract_spectrum(const MSSMatMGUT_mAmu_mass_eigenstates& model)
{
   spectrum.clear();
...
void MSSMatMGUT_mAmu_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MSSMatMGUT_mAmu_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MSSMatMGUT_mAmu_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MSSMatMGUT_mAmu_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MSSMatMGUT_mAmu_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MSSMatMGUT_mAmu_spectrum_plotter::write_spectrum: "
...
namespace MSSMatMGUT_mAmu_database {
 ...
 */
void to_database(
   const std::string& file_name, const MSSMatMGUT_mAmu_mass_eigenstates& model,
   ...
   const MSSMatMGUT_mAmu_observables* observables)
{
   ...
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_DRbar_mass_names());
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_pole_mass_names());
   append(names, MSSMatMGUT_mAmu_parameter_getter().get_pole_mixing_names());
   ...
   if (MSSMatMGUT_mAmu_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MSSMatMGUT_mAmu_observables::get_names());
 ...
 */
MSSMatMGUT_mAmu_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, MSSMatMGUT_mAmu_observables* observables)
{
   using utilities::append;

   MSSMatMGUT_mAmu_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = MSSMatMGUT_mAmu_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = MSSMatMGUT_mAmu_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = MSSMatMGUT_mAmu_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = MSSMatMGUT_mAmu_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? MSSMatMGUT_mAmu_observables::NUMBER_OF_OBSERVABLES : 0);
