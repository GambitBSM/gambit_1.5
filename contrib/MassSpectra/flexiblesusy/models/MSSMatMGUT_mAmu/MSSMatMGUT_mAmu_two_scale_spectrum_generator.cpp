...
// File generated at Sun 24 Sep 2017 15:48:26
...
#include "MSSMatMGUT_mAmu_two_scale_initial_guesser.hpp"
...
double MSSMatMGUT_mAmu_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSMatMGUT_mAmu_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMatMGUT_mAmu_input_parameters& input)
{
   ...
   MSSMatMGUT_mAmu_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMatMGUT_mAmu_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMatMGUT_mAmu_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSMatMGUT_mAmu_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSMatMGUT_mAmu_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSMatMGUT_mAmu_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSMatMGUT_mAmu_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSMatMGUT_mAmu_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMatMGUT_mAmu_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSMatMGUT_mAmu_spectrum_generator<Two_scale>::calculate_spectrum()
{
