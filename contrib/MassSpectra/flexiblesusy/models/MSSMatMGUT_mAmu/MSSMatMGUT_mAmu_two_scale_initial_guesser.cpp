...
MSSMatMGUT_mAmu_initial_guesser<Two_scale>::MSSMatMGUT_mAmu_initial_guesser(
   MSSMatMGUT_mAmu<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const MSSMatMGUT_mAmu_low_scale_constraint<Two_scale>& low_constraint_,
   const MSSMatMGUT_mAmu_susy_scale_constraint<Two_scale>& susy_constraint_,
   const MSSMatMGUT_mAmu_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("MSSMatMGUT_mAmu_initial_guesser: Error: pointer to model"
                       " MSSMatMGUT_mAmu<Two_scale> must not be zero");
}
 ...
 */
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::guess()
{
 ...
 * \code{.cpp}
   ...
   MODEL->set_BMu(Re((0.5*mA2Input*(1 + Sqr(TanBeta)))/TanBeta));

 * \endcode
 */
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * corrections).
 */
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
 ...
 */
void MSSMatMGUT_mAmu_initial_guesser<Two_scale>::guess_soft_parameters()
{
