...
// File generated at Thu 10 May 2018 14:30:26
...
namespace MSSMatMGUT_mAmu_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMatMGUT_mAmuMessage", 1);
...
class MSSMatMGUT_mAmu_spectrum {
public:
   virtual ~MSSMatMGUT_mAmu_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMatMGUT_mAmu_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMatMGUT_mAmu_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMGUT_mAmu_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSMatMGUT_mAmu_spectrum_impl : public MSSMatMGUT_mAmu_spectrum
{
public:
   virtual ~MSSMatMGUT_mAmu_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMatMGUT_mAmu_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMatMGUT_mAmu_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMGUT_mAmu_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSMatMGUT_mAmu_slha<MSSMatMGUT_mAmu<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMatMGUT_mAmu_scales scales{};              ///< scale information
   MSSMatMGUT_mAmu_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSMatMGUT_mAmu_input_parameters& input_) { input = input_; }
...
private:
   MSSMatMGUT_mAmu_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMatMGUT_mAmu_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMatMGUT_mAmu_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSMatMGUT_mAmu_info::particle_names[i].c_str());
      }
   }
}
...
MSSMatMGUT_mAmu_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSMatMGUT_mAmu_slha_io slha_io;
...
void put_spectrum(const MSSMatMGUT_mAmu_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSMatMGUT_mAmu_info::model_name);
...
template <typename Solver_type>
void MSSMatMGUT_mAmu_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMatMGUT_mAmu_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMatMGUT_mAmu_input_parameters& input
)
{
...
template <typename Solver_type>
void MSSMatMGUT_mAmu_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMatMGUT_mAmu_spectrum_impl<Solver_type>::fill_slha_io(MSSMatMGUT_mAmu_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSMatMGUT_mAmu_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMatMGUT_mAmuCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMatMGUT_mAmuCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMatMGUT_mAmu_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMatMGUT_mAmuGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuGetSettings"))
...
DLLEXPORT int FSMSSMatMGUT_mAmuGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUT_mAmuGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUT_mAmuOpenHandle(
...
DLLEXPORT int FSMSSMatMGUT_mAmuCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;
...
DLLEXPORT int FSMSSMatMGUT_mAmuSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;
...
DLLEXPORT int FSMSSMatMGUT_mAmuGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUT_mAmuToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUT_mAmuGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUT_mAmuCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUT_mAmuCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUT_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUT_mAmuCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMatMGUT_mAmuCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMatMGUT_mAmuCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMatMGUT_mAmuCalculateObservables", "error", e.what());
      put_error_output(link);
   }
