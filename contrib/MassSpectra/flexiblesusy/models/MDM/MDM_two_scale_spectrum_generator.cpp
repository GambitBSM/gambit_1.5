...
double MDM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MDM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MDM_input_parameters& input)
{
   ...
   MDM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MDM_ewsb_solver<Two_scale> >(ewsb_solver));

   MDM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MDM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MDM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MDM_convergence_tester<Two_scale> convergence_tester(
   ...
   MDM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MDM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MDM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MDM_spectrum_generator<Two_scale>::calculate_spectrum()
{
