...
// File generated at Wed 4 Apr 2018 09:58:38
...
public:
   ...
   virtual void fill_slha_io(MDM_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MDM_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MDM_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MDM_slha<MDM<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MDM_scales scales{};              ///< scale information
   MDM_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MDM_input_parameters& input_) { input = input_; }
...
private:
   ...
   std::unique_ptr<MDM_spectrum> spectrum{nullptr};  ///< spectrum information

   MDM_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MDM_info::particle_names[i].c_str());
      }
   }
}
...
MDM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MDM_slha_io slha_io;
...
void put_spectrum(const MDM_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MDM_info::model_name);
...
template <typename Solver_type>
void MDM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MDM_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void MDM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MDM_spectrum_impl<Solver_type>::fill_slha_io(MDM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MDM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMDMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMDMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MDM_spectrum_impl<Two_scale>());
...
{
   using namespace flexiblesusy::MDM_librarylink;
...
DLLEXPORT int FSMDMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMDMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMDMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
