...
#include "MDM_weinberg_angle.hpp"
...
void MDM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
...
double MDM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   MDM_weinberg_angle::Sm_parameters sm_pars;
   ...
   MDM_weinberg_angle weinberg(MODEL, sm_pars);
...
double MDM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void MDM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("MDM_low_scale_constraint<Two_scale>: "
