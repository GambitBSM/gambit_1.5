...
#include "MDM_a_muon.hpp"
#include "MDM_mass_eigenstates.hpp"

#include "MDM_cxx_diagrams.hpp"
...
double get_MSUSY(const MDM_mass_eigenstates& model)
{
   return Min(model.get_MFc(), model.get_MFg());

}

void run_to_MSUSY(MDM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MDM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MDM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MDM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MDM_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(MDM_mass_eigenstates& model)
{
   VERBOSE_MSG("MDM_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const MDM_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("MDM_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double MDM_a_muon::calculate_a_muon(const MDM_mass_eigenstates& model_)
{
   MDM_mass_eigenstates model(model_);

   VERBOSE_MSG("MDM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MDM_a_muon:" << e.what());
...
double MDM_a_muon::calculate_a_muon_uncertainty(const MDM_mass_eigenstates& model_)
{
   MDM_mass_eigenstates model(model_);

   VERBOSE_MSG("MDM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MDM_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
