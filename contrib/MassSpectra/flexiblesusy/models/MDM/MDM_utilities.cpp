...
MDM_spectrum_plotter::MDM_spectrum_plotter(const MDM_mass_eigenstates& model)
{
...
void MDM_spectrum_plotter::extract_spectrum(const MDM_mass_eigenstates& model)
{
   ...
   spectrum.emplace_back(TParticle("Fc", "\\chi^{+}", to_valarray(PHYSICAL(MFc))));
   spectrum.emplace_back(TParticle("Fg", "\\chi^{++}", to_valarray(PHYSICAL(MFg))));
   spectrum.emplace_back(TParticle("Fn", "\\chi_0", to_valarray(PHYSICAL(MFn))));
   ...
   const MDM_observables* observables)
{
   ...
   append(names, MDM_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, MDM_parameter_getter().get_DRbar_mass_names());
   append(names, MDM_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, MDM_parameter_getter().get_pole_mass_names());
   append(names, MDM_parameter_getter().get_pole_mixing_names());
   ...
   Physical_input* physical_input, MDM_observables* observables)
{
   ...
   const auto number_of_masses = MDM_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = MDM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = MDM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = MDM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? MDM_observables::NUMBER_OF_OBSERVABLES : 0);
