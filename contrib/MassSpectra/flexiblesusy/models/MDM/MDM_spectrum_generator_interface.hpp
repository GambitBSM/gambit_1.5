...
public:
   virtual ~MDM_spectrum_generator_interface() = default;

   std::tuple<MDM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MDM_slha<MDM<T>>> get_models_slha() const
   { return std::make_tuple(MDM_slha<MDM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   MDM<T> get_model() const
   { return model; }
   MDM_slha<MDM<T>> get_model_slha() const
   { return MDM_slha<MDM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MDM_input_parameters&);
...
protected:
   ...
   virtual void run_except(const softsusy::QedQcd&, const MDM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MDM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MDM_input_parameters& input)
{
 ...
 */
template <class T>
void MDM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MDM_spectrum_plotter plotter(model);
