...
/**
 * @file MDM_cxx_diagrams.hpp
 *
 * This file was generated at Wed 4 Apr 2018 09:58:38 with FlexibleSUSY
 * 2.0.1 and SARAH 4.12.2 .
 */

#ifndef MDM_CXXDIAGRAMS_H
#define MDM_CXXDIAGRAMS_H
...
struct Fg {
   ...
   using smFlags = boost::mpl::vector_c<bool, false>;
   static constexpr int numberOfFieldIndices = 0;
   static constexpr double electric_charge = 2;
   using lorentz_conjugate = bar<Fg>::type;
};
...
template<> struct bar<Fn> { using type = Fn; };
...
*/
struct EvaluationContext {
   MDM_mass_eigenstates& model; ///< The model object.
...
template<> inline
double EvaluationContext::mass_impl<Fc>(const std::array<int, 0>& indices) const
{ return model.get_MFc(); }

template<> inline
double EvaluationContext::mass_impl<Fg>(const std::array<int, 0>& indices) const
{ return model.get_MFg(); }

template<> inline
double EvaluationContext::mass_impl<Fn>(const std::array<int, 0>& indices) const
{ return model.get_MFn(); }
