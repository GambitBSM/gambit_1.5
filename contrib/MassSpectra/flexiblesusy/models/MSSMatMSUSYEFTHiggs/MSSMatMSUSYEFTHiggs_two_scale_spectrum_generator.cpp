...
#include "MSSMatMSUSYEFTHiggs_two_scale_convergence_tester.hpp"
#include "MSSMatMSUSYEFTHiggs_two_scale_ewsb_solver.hpp"
#include "MSSMatMSUSYEFTHiggs_two_scale_initial_guesser.hpp"
#include "MSSMatMSUSYEFTHiggs_two_scale_susy_scale_constraint.hpp"
#include "MSSMatMSUSYEFTHiggs_standard_model_matching.hpp"
#include "MSSMatMSUSYEFTHiggs_standard_model_two_scale_matching.hpp"
...
double MSSMatMSUSYEFTHiggs_spectrum_generator<Two_scale>::get_pole_mass_scale(double susy_scale) const
{
...
double MSSMatMSUSYEFTHiggs_spectrum_generator<Two_scale>::get_eft_pole_mass_scale(double susy_scale, double Mt) const
{
   double Q_higgs =
 ...
 */
void MSSMatMSUSYEFTHiggs_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMatMSUSYEFTHiggs_input_parameters& input)
{
   ...
   MSSMatMSUSYEFTHiggs_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMatMSUSYEFTHiggs_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMatMSUSYEFTHiggs_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   ...
   MSSMatMSUSYEFTHiggs_standard_model_matching_up<Two_scale> matching_up(
      &eft, &model, scale_getter, matching_loop_order_up, index);
   MSSMatMSUSYEFTHiggs_standard_model_matching_down<Two_scale> matching_down(
   ...
   // convergence tester for MSSMatMSUSYEFTHiggs
   MSSMatMSUSYEFTHiggs_convergence_tester<Two_scale> model_ct(
   ...
   MSSMatMSUSYEFTHiggs_standard_model_initial_guesser<Two_scale> initial_guesser(&model, &eft, qedqcd,
 ...
 */
void MSSMatMSUSYEFTHiggs_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMatMSUSYEFTHiggs_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void MSSMatMSUSYEFTHiggs_spectrum_generator<Two_scale>::calculate_spectrum(double Mt, double MW)
{
   ...
   if (eft.get_problems().is_running_tachyon(standard_model_info::hh))
      model.get_problems().flag_running_tachyon(MSSMatMSUSYEFTHiggs_info::hh);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::hh))
      model.get_problems().flag_pole_tachyon(MSSMatMSUSYEFTHiggs_info::hh);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VZ))
      model.get_problems().flag_running_tachyon(MSSMatMSUSYEFTHiggs_info::VZ);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VZ))
      model.get_problems().flag_pole_tachyon(MSSMatMSUSYEFTHiggs_info::VZ);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VWp))
      model.get_problems().flag_running_tachyon(MSSMatMSUSYEFTHiggs_info::VWm);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VWp))
      model.get_problems().flag_pole_tachyon(MSSMatMSUSYEFTHiggs_info::VWm);
}
