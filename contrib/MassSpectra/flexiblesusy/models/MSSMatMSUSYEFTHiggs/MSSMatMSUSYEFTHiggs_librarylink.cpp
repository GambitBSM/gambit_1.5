...
// File generated at Thu 10 May 2018 14:39:22

#include "config.h"

#include "MSSMatMSUSYEFTHiggs_info.hpp"
#include "MSSMatMSUSYEFTHiggs_input_parameters.hpp"
#include "MSSMatMSUSYEFTHiggs_observables.hpp"
#include "MSSMatMSUSYEFTHiggs_physical.hpp"
#include "MSSMatMSUSYEFTHiggs_slha_io.hpp"
#include "MSSMatMSUSYEFTHiggs_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "MSSMatMSUSYEFTHiggs_two_scale_model.hpp"
#include "MSSMatMSUSYEFTHiggs_two_scale_spectrum_generator.hpp"
#endif
...
namespace MSSMatMSUSYEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMatMSUSYEFTHiggsMessage", 1);
...
class MSSMatMSUSYEFTHiggs_spectrum {
public:
   virtual ~MSSMatMSUSYEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMatMSUSYEFTHiggs_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMatMSUSYEFTHiggs_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMSUSYEFTHiggs_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSMatMSUSYEFTHiggs_spectrum_impl : public MSSMatMSUSYEFTHiggs_spectrum
{
public:
   virtual ~MSSMatMSUSYEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMatMSUSYEFTHiggs_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMatMSUSYEFTHiggs_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMSUSYEFTHiggs_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSMatMSUSYEFTHiggs_slha<MSSMatMSUSYEFTHiggs<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMatMSUSYEFTHiggs_scales scales{};              ///< scale information
   MSSMatMSUSYEFTHiggs_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSMatMSUSYEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   MSSMatMSUSYEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMatMSUSYEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMatMSUSYEFTHiggs_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSMatMSUSYEFTHiggs_info::particle_names[i].c_str());
      }
   }
}
...
MSSMatMSUSYEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSMatMSUSYEFTHiggs_slha_io slha_io;
...
void put_spectrum(const MSSMatMSUSYEFTHiggs_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSMatMSUSYEFTHiggs_info::model_name);
...
template <typename Solver_type>
void MSSMatMSUSYEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMatMSUSYEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMatMSUSYEFTHiggs_input_parameters& input
)
{
   MSSMatMSUSYEFTHiggs_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMatMSUSYEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMatMSUSYEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(MSSMatMSUSYEFTHiggs_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSMatMSUSYEFTHiggs_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMatMSUSYEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMatMSUSYEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMatMSUSYEFTHiggs_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   MSSMatMSUSYEFTHiggs_input_parameters input;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsGetSettings"))
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsOpenHandle(
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSYEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSYEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSYEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMatMSUSYEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMatMSUSYEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMatMSUSYEFTHiggsCalculateObservables", "error", e.what());
      put_error_output(link);
   }
