...
// File generated at Thu 10 May 2018 14:35:07

#include "config.h"

#include "MSSMatMGUTEFTHiggs_mAmu_info.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_input_parameters.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_observables.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_physical.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_slha_io.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "MSSMatMGUTEFTHiggs_mAmu_two_scale_model.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_two_scale_spectrum_generator.hpp"
#endif
...
namespace MSSMatMGUTEFTHiggs_mAmu_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMatMGUTEFTHiggs_mAmuMessage", 1);
...
class MSSMatMGUTEFTHiggs_mAmu_spectrum {
public:
   virtual ~MSSMatMGUTEFTHiggs_mAmu_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMatMGUTEFTHiggs_mAmu_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMatMGUTEFTHiggs_mAmu_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMGUTEFTHiggs_mAmu_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSMatMGUTEFTHiggs_mAmu_spectrum_impl : public MSSMatMGUTEFTHiggs_mAmu_spectrum
{
public:
   virtual ~MSSMatMGUTEFTHiggs_mAmu_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMatMGUTEFTHiggs_mAmu_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMatMGUTEFTHiggs_mAmu_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMGUTEFTHiggs_mAmu_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSMatMGUTEFTHiggs_mAmu_slha<MSSMatMGUTEFTHiggs_mAmu<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMatMGUTEFTHiggs_mAmu_scales scales{};              ///< scale information
   MSSMatMGUTEFTHiggs_mAmu_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSMatMGUTEFTHiggs_mAmu_input_parameters& input_) { input = input_; }
...
private:
   MSSMatMGUTEFTHiggs_mAmu_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMatMGUTEFTHiggs_mAmu_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMatMGUTEFTHiggs_mAmu_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSMatMGUTEFTHiggs_mAmu_info::particle_names[i].c_str());
      }
   }
}
...
MSSMatMGUTEFTHiggs_mAmu_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSMatMGUTEFTHiggs_mAmu_slha_io slha_io;
...
void put_spectrum(const MSSMatMGUTEFTHiggs_mAmu_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSMatMGUTEFTHiggs_mAmu_info::model_name);
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_mAmu_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_mAmu_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMatMGUTEFTHiggs_mAmu_input_parameters& input
)
{
   MSSMatMGUTEFTHiggs_mAmu_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_mAmu_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_mAmu_spectrum_impl<Solver_type>::fill_slha_io(MSSMatMGUTEFTHiggs_mAmu_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSMatMGUTEFTHiggs_mAmu_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMatMGUTEFTHiggs_mAmuCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMatMGUTEFTHiggs_mAmuCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMatMGUTEFTHiggs_mAmu_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuGetSettings"))
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuOpenHandle(
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggs_mAmuCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggs_mAmuCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMatMGUTEFTHiggs_mAmuCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMatMGUTEFTHiggs_mAmuCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMatMGUTEFTHiggs_mAmuCalculateObservables", "error", e.what());
      put_error_output(link);
   }
