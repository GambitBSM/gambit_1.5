...
#include "MSSMatMGUTEFTHiggs_mAmu_two_scale_convergence_tester.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_two_scale_ewsb_solver.hpp"
#include "MSSMatMGUTEFTHiggs_mAmu_two_scale_initial_guesser.hpp"
...
#include "MSSMatMGUTEFTHiggs_mAmu_standard_model_two_scale_matching.hpp"
...
double MSSMatMGUTEFTHiggs_mAmu_spectrum_generator<Two_scale>::get_pole_mass_scale(double susy_scale) const
{
...
double MSSMatMGUTEFTHiggs_mAmu_spectrum_generator<Two_scale>::get_eft_pole_mass_scale(double susy_scale, double Mt) const
{
 ...
 */
void MSSMatMGUTEFTHiggs_mAmu_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMatMGUTEFTHiggs_mAmu_input_parameters& input)
{
   ...
   MSSMatMGUTEFTHiggs_mAmu_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMatMGUTEFTHiggs_mAmu_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMatMGUTEFTHiggs_mAmu_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSMatMGUTEFTHiggs_mAmu_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   ...
   MSSMatMGUTEFTHiggs_mAmu_standard_model_matching_up<Two_scale> matching_up(
      &eft, &model, scale_getter, matching_loop_order_up, index);
   MSSMatMGUTEFTHiggs_mAmu_standard_model_matching_down<Two_scale> matching_down(
   ...
   // convergence tester for MSSMatMGUTEFTHiggs_mAmu
   MSSMatMGUTEFTHiggs_mAmu_convergence_tester<Two_scale> model_ct(
   ...
   MSSMatMGUTEFTHiggs_mAmu_standard_model_initial_guesser<Two_scale> initial_guesser(&model, &eft, qedqcd,
 ...
 */
void MSSMatMGUTEFTHiggs_mAmu_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMatMGUTEFTHiggs_mAmu_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSMatMGUTEFTHiggs_mAmu_spectrum_generator<Two_scale>::calculate_spectrum(double Mt, double MW)
{
   ...
   if (eft.get_problems().is_running_tachyon(standard_model_info::hh))
      model.get_problems().flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::hh);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::hh))
      model.get_problems().flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::hh);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VZ))
      model.get_problems().flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VZ);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VZ))
      model.get_problems().flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VZ);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VWp))
      model.get_problems().flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VWm);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VWp))
      model.get_problems().flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VWm);
}
