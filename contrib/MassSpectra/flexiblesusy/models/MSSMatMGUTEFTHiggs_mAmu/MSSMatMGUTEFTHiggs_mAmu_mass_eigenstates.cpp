...
// File generated at Thu 10 May 2018 14:34:48

/**
 * @file MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates.cpp
 * @brief implementation of the MSSMatMGUTEFTHiggs_mAmu model class
 *
 * Contains the definition of the MSSMatMGUTEFTHiggs_mAmu model class methods
 * which solve EWSB and calculate pole masses and mixings from DRbar
 * parameters.
 *
 * This file was generated at Thu 10 May 2018 14:34:48 with FlexibleSUSY
...
#include "MSSMatMGUTEFTHiggs_mAmu_two_scale_ewsb_solver.hpp"
#endif
...
#define CLASSNAME MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates
...
CLASSNAME::MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates(const MSSMatMGUTEFTHiggs_mAmu_input_parameters& input_)
   : MSSMatMGUTEFTHiggs_mAmu_soft_parameters(input_)
#if defined(ENABLE_TWO_SCALE_SOLVER)
   , ewsb_solver(new MSSMatMGUTEFTHiggs_mAmu_ewsb_solver<Two_scale>())
#endif
{
}
...
const MSSMatMGUTEFTHiggs_mAmu_physical& CLASSNAME::get_physical() const
{
   return physical;
}

MSSMatMGUTEFTHiggs_mAmu_physical& CLASSNAME::get_physical()
{
   return physical;
}

void CLASSNAME::set_physical(const MSSMatMGUTEFTHiggs_mAmu_physical& physical_)
{
   physical = physical_;
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<MSSMatMGUTEFTHiggs_mAmu_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving MSSMatMGUTEFTHiggs_mAmu EWSB at " << ewsb_loop_order << "-loop order");
...
{
   ostr << "========================================\n"
           "MSSMatMGUTEFTHiggs_mAmu\n"
           "========================================\n";
   MSSMatMGUTEFTHiggs_mAmu_soft_parameters::print(ostr);
   ostr << "----------------------------------------\n"
...
{
   if (PHYSICAL(MSd).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Sd);
   if (PHYSICAL(MSv).tail<3>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Sv);
   if (PHYSICAL(MSu).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Su);
   if (PHYSICAL(MSe).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Se);
   if (PHYSICAL(Mhh).tail<2>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::hh);
   if (PHYSICAL(MAh).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Ah);
   if (PHYSICAL(MHpm).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Hpm);
...
{
   MSSMatMGUTEFTHiggs_mAmu_soft_parameters::clear();
   clear_DRbar_parameters();
   physical.clear();
   problems.clear();
}
...
{
   return "MSSMatMGUTEFTHiggs_mAmu";
}
...
void CLASSNAME::run_to(double scale, double eps)
{
   if (eps < 0.0)
      eps = precision;
   MSSMatMGUTEFTHiggs_mAmu_soft_parameters::run_to(scale, eps);
}
 ...
 * @return mass of LSP
 */
double CLASSNAME::get_lsp(MSSMatMGUTEFTHiggs_mAmu_info::Particles& particle_type) const
{
   double lsp_mass = std::numeric_limits<double>::max();
   double tmp_mass;
   particle_type = MSSMatMGUTEFTHiggs_mAmu_info::NUMBER_OF_PARTICLES;

   tmp_mass = Abs(PHYSICAL(MChi(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Chi;
   }

   tmp_mass = Abs(PHYSICAL(MSv(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Sv;
   }

   tmp_mass = Abs(PHYSICAL(MSu(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Su;
   }

   tmp_mass = Abs(PHYSICAL(MSd(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Sd;
   }

   tmp_mass = Abs(PHYSICAL(MSe(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Se;
   }

   tmp_mass = Abs(PHYSICAL(MCha(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Cha;
   }

   tmp_mass = Abs(PHYSICAL(MGlu));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = MSSMatMGUTEFTHiggs_mAmu_info::Glu;
   }
...
void CLASSNAME::calculate_MSd()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Sd,
   ...
   if (MSd.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Sd);
   }
...
void CLASSNAME::calculate_MSv()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Sv,
      eigenvalue_error > precision * Abs(MSv(0)));
#else
   ...
   if (MSv.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Sv);
   }
...
void CLASSNAME::calculate_MSu()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Su,
   ...
   if (MSu.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Su);
   }
...
void CLASSNAME::calculate_MSe()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Se,
   ...
   if (MSe.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Se);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::hh,
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Ah,
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Ah);
   }
...
void CLASSNAME::calculate_MHpm()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Hpm,
   ...
   if (MHpm.minCoeff() < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::Hpm)
         ;
   }
...
void CLASSNAME::calculate_MChi()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Chi,
...
void CLASSNAME::calculate_MCha()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Cha,
...
void CLASSNAME::calculate_MFe()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Fe,
...
void CLASSNAME::calculate_MFd()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Fd,
...
void CLASSNAME::calculate_MFu()
{
   ...
   problems.flag_bad_mass(MSSMatMGUTEFTHiggs_mAmu_info::Fu,
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_running_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VWm)
         ;
   }
...
void CLASSNAME::calculate_MVG_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::VG);
   else
...
void CLASSNAME::calculate_MGlu_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Glu);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Glu);
}
...
void CLASSNAME::calculate_MFv_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fv);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fv);
}
...
void CLASSNAME::calculate_MVP_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::VP);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::VP);
}
...
void CLASSNAME::calculate_MVZ_pole()
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::VZ))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::Sd))
      return;
      ...
      for (int es = 0; es < 6; ++es) {
         ...
         Eigen::Matrix<double,6,6> mix_ZD;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Sd, eigenvalue_error > precision *
               Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZD);
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::Sv))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 3; ++es) {
         ...
         Eigen::Matrix<double,3,3> mix_ZV;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Sv, eigenvalue_error > precision *
               Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZV);
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::Su))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 6; ++es) {
         ...
         Eigen::Matrix<double,6,6> mix_ZU;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Su, eigenvalue_error > precision *
               Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZU);
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::Se))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 6; ++es) {
         ...
         Eigen::Matrix<double,6,6> mix_ZE;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Se, eigenvalue_error > precision *
               Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZE);
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::hh))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZH;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::hh, eigenvalue_error > precision *
               Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZH);
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::Ah))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZA;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Ah, eigenvalue_error > precision *
               Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZA);
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::Hpm))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZP;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Hpm, eigenvalue_error > precision
               * Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZP);
...
void CLASSNAME::calculate_MChi_pole()
{
   ...
   do {
      ...
      for (int es = 0; es < 4; ++es) {
         ...
         decltype(ZN) mix_ZN;
            ...
            problems.flag_bad_mass(
               MSSMatMGUTEFTHiggs_mAmu_info::Chi, eigenvalue_error > precision
               * Abs(eigen_values(0)));
         #else
            fs_diagonalize_symmetric(M_loop, eigen_values,
               mix_ZN);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Chi);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Chi);
}
...
void CLASSNAME::calculate_MCha_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Cha);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Cha);
}
...
void CLASSNAME::calculate_MFe_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fe);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fe);
}
...
void CLASSNAME::calculate_MFd_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fd);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fd);
}
...
void CLASSNAME::calculate_MFu_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fu);
   else
      problems.unflag_no_pole_mass_convergence(
         MSSMatMGUTEFTHiggs_mAmu_info::Fu);
}
...
{
   if (!force_output && problems.is_running_tachyon(
      MSSMatMGUTEFTHiggs_mAmu_info::VWm))
      return;
...
double CLASSNAME::calculate_MVWm_pole(double p)
{
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VWm);
...
double CLASSNAME::calculate_MVZ_pole(double p)
{
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(MSSMatMGUTEFTHiggs_mAmu_info::VZ);
...
std::ostream& operator<<(std::ostream& ostr, const MSSMatMGUTEFTHiggs_mAmu_mass_eigenstates& model)
{
   model.print(ostr);
   return ostr;
}
