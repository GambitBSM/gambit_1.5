...
#include "MSSMatMSUSY_mAmu_utilities.hpp"
...
MSSMatMSUSY_mAmu_spectrum_plotter::MSSMatMSUSY_mAmu_spectrum_plotter(const MSSMatMSUSY_mAmu_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void MSSMatMSUSY_mAmu_spectrum_plotter::extract_spectrum(const MSSMatMSUSY_mAmu_mass_eigenstates& model)
{
   spectrum.clear();
...
void MSSMatMSUSY_mAmu_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MSSMatMSUSY_mAmu_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MSSMatMSUSY_mAmu_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MSSMatMSUSY_mAmu_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MSSMatMSUSY_mAmu_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MSSMatMSUSY_mAmu_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const MSSMatMSUSY_mAmu_mass_eigenstates& model,
   ...
   const MSSMatMSUSY_mAmu_observables* observables)
{
   ...
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_DRbar_mass_names());
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_pole_mass_names());
   append(names, MSSMatMSUSY_mAmu_parameter_getter().get_pole_mixing_names());
   ...
   if (MSSMatMSUSY_mAmu_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MSSMatMSUSY_mAmu_observables::get_names());
...
MSSMatMSUSY_mAmu_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, MSSMatMSUSY_mAmu_observables* observables)
{
   using utilities::append;

   MSSMatMSUSY_mAmu_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = MSSMatMSUSY_mAmu_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = MSSMatMSUSY_mAmu_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = MSSMatMSUSY_mAmu_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = MSSMatMSUSY_mAmu_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? MSSMatMSUSY_mAmu_observables::NUMBER_OF_OBSERVABLES : 0);
