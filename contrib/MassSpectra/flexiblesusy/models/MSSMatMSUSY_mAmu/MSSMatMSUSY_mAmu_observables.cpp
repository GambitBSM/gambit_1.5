...
#include "MSSMatMSUSY_mAmu_edm.hpp"
...
const int MSSMatMSUSY_mAmu_observables::NUMBER_OF_OBSERVABLES;

MSSMatMSUSY_mAmu_observables::MSSMatMSUSY_mAmu_observables()
   : a_muon(0)
...
Eigen::ArrayXd MSSMatMSUSY_mAmu_observables::get() const
{
   Eigen::ArrayXd vec(MSSMatMSUSY_mAmu_observables::NUMBER_OF_OBSERVABLES);
...
std::vector<std::string> MSSMatMSUSY_mAmu_observables::get_names()
{
   std::vector<std::string> names(MSSMatMSUSY_mAmu_observables::NUMBER_OF_OBSERVABLES);
...
void MSSMatMSUSY_mAmu_observables::clear()
{
   a_muon = 0.;
   eff_cp_higgs_photon_photon = Eigen::Array<std::complex<double>,2,1>::Zero();
   eff_cp_higgs_gluon_gluon = Eigen::Array<std::complex<double>,2,1>::Zero();
...
void MSSMatMSUSY_mAmu_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == MSSMatMSUSY_mAmu_observables::NUMBER_OF_OBSERVABLES);
...
MSSMatMSUSY_mAmu_observables calculate_observables(const MSSMatMSUSY_mAmu_mass_eigenstates& model,
...
MSSMatMSUSY_mAmu_observables calculate_observables(const MSSMatMSUSY_mAmu_mass_eigenstates& model,
                                              ...
                                              const Physical_input& physical_input)
{
   MSSMatMSUSY_mAmu_observables observables;

   MSSMatMSUSY_mAmu_effective_couplings effective_couplings(model, qedqcd, physical_input);
   effective_couplings.calculate_effective_couplings();

   observables.AMU = MSSMatMSUSY_mAmu_a_muon::calculate_a_muon(MODEL);
