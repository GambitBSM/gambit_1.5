...
// File generated at Thu 28 Sep 2017 14:27:53
...
MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::MSSMatMSUSY_mAmu_low_scale_constraint(
   MSSMatMSUSY_mAmu<Two_scale>* model_, const softsusy::QedQcd& qedqcd_)
   : model(model_)
   , qedqcd(qedqcd_)
{
   initialize();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
const Eigen::Matrix<std::complex<double>,3,3>& MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<MSSMatMSUSY_mAmu<Two_scale>*>(model_);
}

void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   MSSMatMSUSY_mAmu_weinberg_angle::Sm_parameters sm_pars;
   ...
   MSSMatMSUSY_mAmu_weinberg_angle weinberg(MODEL, sm_pars);
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
   ...
   if (IsFinite(new_g1)) {
      model->get_problems().unflag_non_perturbative_parameter(
         MSSMatMSUSY_mAmu_info::g1);
   } else {
      model->get_problems().flag_non_perturbative_parameter(
         MSSMatMSUSY_mAmu_info::g1, new_g1, get_scale());
      new_g1 = Electroweak_constants::g1;
   }

   if (IsFinite(new_g2)) {
      model->get_problems().unflag_non_perturbative_parameter(
         MSSMatMSUSY_mAmu_info::g2);
   } else {
      model->get_problems().flag_non_perturbative_parameter(
         MSSMatMSUSY_mAmu_info::g2, new_g2, get_scale());
      new_g2 = Electroweak_constants::g2;
   }

}

double MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::calculate_MNeutrino_DRbar()
{
   neutrinoDRbar.setZero();
...
void MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("MSSMatMSUSY_mAmu_low_scale_constraint<Two_scale>: "
