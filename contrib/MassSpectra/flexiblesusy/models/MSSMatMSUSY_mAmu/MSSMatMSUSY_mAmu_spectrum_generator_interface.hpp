...
template <class T>
class MSSMatMSUSY_mAmu_spectrum_generator_interface {
public:
   virtual ~MSSMatMSUSY_mAmu_spectrum_generator_interface() = default;

   std::tuple<MSSMatMSUSY_mAmu<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSMatMSUSY_mAmu_slha<MSSMatMSUSY_mAmu<T>>> get_models_slha() const
   { return std::make_tuple(MSSMatMSUSY_mAmu_slha<MSSMatMSUSY_mAmu<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   MSSMatMSUSY_mAmu<T> get_model() const
   { return model; }
   MSSMatMSUSY_mAmu_slha<MSSMatMSUSY_mAmu<T>> get_model_slha() const
   { return MSSMatMSUSY_mAmu_slha<MSSMatMSUSY_mAmu<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMatMSUSY_mAmu_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMatMSUSY_mAmu_spectrum.dat") const;

protected:
   MSSMatMSUSY_mAmu<T> model;
   ...
   void translate_exception_to_problem(MSSMatMSUSY_mAmu<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMatMSUSY_mAmu_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMatMSUSY_mAmu_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMatMSUSY_mAmu_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMatMSUSY_mAmu_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMatMSUSY_mAmu_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMatMSUSY_mAmu_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMatMSUSY_mAmu_parameter_getter parameter_getter;
   Coupling_monitor<MSSMatMSUSY_mAmu_mass_eigenstates, MSSMatMSUSY_mAmu_parameter_getter>
 ...
 */
template <class T>
void MSSMatMSUSY_mAmu_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMatMSUSY_mAmu<T>& model)
{
   try {
      throw;
