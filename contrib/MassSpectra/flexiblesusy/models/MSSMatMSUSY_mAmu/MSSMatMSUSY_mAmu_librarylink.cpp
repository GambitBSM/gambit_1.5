...
// File generated at Wed 25 Oct 2017 18:00:16
...
namespace MSSMatMSUSY_mAmu_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMatMSUSY_mAmuMessage", 1);
...
class MSSMatMSUSY_mAmu_spectrum {
public:
   virtual ~MSSMatMSUSY_mAmu_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMatMSUSY_mAmu_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMatMSUSY_mAmu_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMSUSY_mAmu_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSMatMSUSY_mAmu_spectrum_impl : public MSSMatMSUSY_mAmu_spectrum
{
public:
   virtual ~MSSMatMSUSY_mAmu_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMatMSUSY_mAmu_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMatMSUSY_mAmu_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMSUSY_mAmu_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSMatMSUSY_mAmu_slha<MSSMatMSUSY_mAmu<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMatMSUSY_mAmu_scales scales{};              ///< scale information
   MSSMatMSUSY_mAmu_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSMatMSUSY_mAmu_input_parameters& input_) { input = input_; }
...
private:
   MSSMatMSUSY_mAmu_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMatMSUSY_mAmu_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMatMSUSY_mAmu_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSMatMSUSY_mAmu_info::particle_names[i].c_str());
      }
   }
}
...
MSSMatMSUSY_mAmu_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSMatMSUSY_mAmu_slha_io slha_io;
...
void put_spectrum(const MSSMatMSUSY_mAmu_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSMatMSUSY_mAmu_info::model_name);
...
template <typename Solver_type>
void MSSMatMSUSY_mAmu_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMatMSUSY_mAmu_spectrum_impl<Solver_type>::calculate_spectrum(
...
{
   MSSMatMSUSY_mAmu_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMatMSUSY_mAmu_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMatMSUSY_mAmu_spectrum_impl<Solver_type>::fill_slha_io(MSSMatMSUSY_mAmu_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSMatMSUSY_mAmu_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMatMSUSY_mAmuCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMatMSUSY_mAmuCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMatMSUSY_mAmu_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMatMSUSY_mAmuGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuGetSettings"))
...
DLLEXPORT int FSMSSMatMSUSY_mAmuGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuOpenHandle(
...
DLLEXPORT int FSMSSMatMSUSY_mAmuCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMSUSY_mAmuCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMSUSY_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMSUSY_mAmuCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMatMSUSY_mAmuCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMatMSUSY_mAmuCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMatMSUSY_mAmuCalculateObservables", "error", e.what());
      put_error_output(link);
   }
