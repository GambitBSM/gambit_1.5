...
namespace ScalarSingletDM_Z3_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSScalarSingletDM_Z3Message", 1);
...
class ScalarSingletDM_Z3_spectrum {
public:
   virtual ~ScalarSingletDM_Z3_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(ScalarSingletDM_Z3_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const ScalarSingletDM_Z3_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const ScalarSingletDM_Z3_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class ScalarSingletDM_Z3_spectrum_impl : public ScalarSingletDM_Z3_spectrum
{
public:
   virtual ~ScalarSingletDM_Z3_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(ScalarSingletDM_Z3_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const ScalarSingletDM_Z3_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const ScalarSingletDM_Z3_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<ScalarSingletDM_Z3_slha<ScalarSingletDM_Z3<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   ScalarSingletDM_Z3_scales scales{};              ///< scale information
   ScalarSingletDM_Z3_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const ScalarSingletDM_Z3_input_parameters& input_) { input = input_; }
...
private:
   ScalarSingletDM_Z3_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<ScalarSingletDM_Z3_spectrum> spectrum{nullptr};  ///< spectrum information

   ScalarSingletDM_Z3_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, ScalarSingletDM_Z3_info::particle_names[i].c_str());
      }
   }
}
...
ScalarSingletDM_Z3_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const ScalarSingletDM_Z3_slha<Model>& model, MLINK link)
{
   MLPutRule(link, ScalarSingletDM_Z3_info::model_name);
   MLPutFunction(link, "List", 52);
   ...
   MLPutRuleTo(link, MODELPARAMETER(Yd), "Yd");
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(mu3), "mu3");
...
template <typename Solver_type>
void ScalarSingletDM_Z3_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void ScalarSingletDM_Z3_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const ScalarSingletDM_Z3_input_parameters& input
)
{
...
template <typename Solver_type>
void ScalarSingletDM_Z3_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void ScalarSingletDM_Z3_spectrum_impl<Solver_type>::fill_slha_io(ScalarSingletDM_Z3_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   ScalarSingletDM_Z3_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSScalarSingletDM_Z3CalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSScalarSingletDM_Z3CalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new ScalarSingletDM_Z3_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSScalarSingletDM_Z3GetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3GetSettings"))
...
DLLEXPORT int FSScalarSingletDM_Z3GetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3GetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z3GetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3GetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z3OpenHandle(
...
DLLEXPORT int FSScalarSingletDM_Z3CloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;
...
DLLEXPORT int FSScalarSingletDM_Z3Set(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;
...
DLLEXPORT int FSScalarSingletDM_Z3GetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3GetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z3ToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3ToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z3GetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3GetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z3CalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3CalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSScalarSingletDM_Z3CalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::ScalarSingletDM_Z3_librarylink;

   if (!check_number_of_args(link, 1, "FSScalarSingletDM_Z3CalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSScalarSingletDM_Z3CalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSScalarSingletDM_Z3CalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSScalarSingletDM_Z3CalculateObservables", "error", e.what());
      put_error_output(link);
   }
