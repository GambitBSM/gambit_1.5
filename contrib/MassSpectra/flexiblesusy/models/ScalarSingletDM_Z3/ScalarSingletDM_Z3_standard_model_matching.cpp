...
#include "ScalarSingletDM_Z3_standard_model_matching.hpp"
 ...
 * lambda is calculated from the tree-level ScalarSingletDM_Z3 parameters.
 *
 * @param sm SM parameters
 * @param model_0l ScalarSingletDM_Z3 tree-level parameters
...
Standard_model calculate_SM_tree_level(
   ...
   const ScalarSingletDM_Z3_mass_eigenstates& model_0l,
   int idx)
{
   auto sm_0l = sm;
...
/**
 * Calculates ScalarSingletDM_Z3 tree-level parameters (g1, g2, g3, Yu, Yd,
 * Ye, v) by performing a tree-level matching from the SM to the
 * ScalarSingletDM_Z3.
 *
 * @param model ScalarSingletDM_Z3 parameters
 * @param sm SM parameters
 *
 * @return ScalarSingletDM_Z3 with tree-level parameters
 */
ScalarSingletDM_Z3_mass_eigenstates calculate_ScalarSingletDM_Z3_tree_level(
   const ScalarSingletDM_Z3_mass_eigenstates& model,
   const Standard_model& sm)
{
   auto model_0l = model;
...
Eigen::Matrix<double,1,1> calculate_tadpole_over_vevs(
   ScalarSingletDM_Z3_mass_eigenstates model, int loop_order)
{
   if (loop_order == 0) {
...
/**
 * Calculates tree-level Higgs mass matrix in the ScalarSingletDM_Z3 without
 ...
 *
 * @param model ScalarSingletDM_Z3 parameters
 * @return tree-level Higgs mass matrix in the ScalarSingletDM_Z3 w/o tadpoles
 */
auto calculate_mh2_tree_level(ScalarSingletDM_Z3_mass_eigenstates model) -> decltype(model.get_mass_matrix_hh())
{
...
/**
 * Calculates squared Higgs pole mass in the ScalarSingletDM_Z3,
 * \f$(M_h^{\text{ScalarSingletDM_Z3}})^2\f$.
 *
 * @param model_0l tree-level ScalarSingletDM_Z3 parameters
 * @param model_1l 1-loop ScalarSingletDM_Z3 parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 *
 * @return squared Higgs pole mass in the ScalarSingletDM_Z3
 */
double calculate_Mh2_pole(
   const ScalarSingletDM_Z3_mass_eigenstates& model_0l,
   const ScalarSingletDM_Z3_mass_eigenstates& model_1l,
   int idx)
{
 ...
 * lightest CP-even Higgs boson mass of the ScalarSingletDM_Z3 by requiring
 * that the Higgs pole masses are equal in both models.
 *
 * @param sm Standard Model
 * @param model_1l ScalarSingletDM_Z3 parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void match_high_to_low_scale_model_1loop(
   ...
   int idx)
{
   // tree-level ScalarSingletDM_Z3 parameters
   const auto model_0l = calculate_ScalarSingletDM_Z3_tree_level(model_1l, sm);
...
double calculate_delta_alpha_em(double alpha_em, const ScalarSingletDM_Z3_mass_eigenstates& model)
{
...
double calculate_delta_alpha_s(double alpha_s, const ScalarSingletDM_Z3_mass_eigenstates& model)
{
   const double currentScale = model.get_scale();
...
Eigen::Matrix<double,3,3> calculate_MFu_DRbar_tree_level(const ScalarSingletDM_Z3_mass_eigenstates& model)
{
...
Eigen::Matrix<double,3,3> calculate_MFd_DRbar_tree_level(const ScalarSingletDM_Z3_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
Eigen::Matrix<double,3,3> calculate_MFe_DRbar_tree_level(const ScalarSingletDM_Z3_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
double calculate_MW_pole_1loop(const ScalarSingletDM_Z3_mass_eigenstates& model_0l)
{
...
double calculate_MZ_pole_1loop(const ScalarSingletDM_Z3_mass_eigenstates& model_0l)
{
...
/**
 * Calculates ScalarSingletDM_Z3 parameters at 1-loop level by performing a
 * 1-loop matching.
 *
 * @param sm_0l SM parameters (lambda is at tree-level)
 * @param sm_1l SM parameters (level is at 1-loop level)
 * @param model_0l ScalarSingletDM_Z3 parameters at tree-level
 * @param model_1l ScalarSingletDM_Z3 parameters at 1-loop level
 *
 * @return ScalarSingletDM_Z3 1-loop parameters
 */
ScalarSingletDM_Z3_mass_eigenstates calculate_ScalarSingletDM_Z3_1loop(
   const Standard_model& sm_0l,
   ...
   const ScalarSingletDM_Z3_mass_eigenstates& model_1l)
{
   ...
   // running ScalarSingletDM_Z3 W, Z masses (via 1L matching)
   ...
   // running ScalarSingletDM_Z3 quark and lepton masses (via 1L matching)
   ...
   // running ScalarSingletDM_Z3 gauge couplings (via 1L matching)
   const double g1_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) * mZ2_1L / mW2_1L) / ScalarSingletDM_Z3_info::normalization_g1;
   const double g2_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) / (1. - mW2_1L/mZ2_1L)) / ScalarSingletDM_Z3_info::normalization_g2;
   const double g3_1L = AbsSqrt(4. * Pi * alpha_s * (1. + delta_alpha_s)) / ScalarSingletDM_Z3_info::normalization_g3;
   ...
   {
      auto MODEL = &model;
      const double VEV = 2. * AbsSqrt(mZ2_1L/(Sqr(g1_1L*ScalarSingletDM_Z3_info::normalization_g1) + Sqr(g2_1L*ScalarSingletDM_Z3_info::normalization_g2)));
 ...
 * the ScalarSingletDM_Z3 at the necessary loop level from the known Standard
 * Model couplings and the SM vev.
 *
 * @param model ScalarSingletDM_Z3 parameters (to be set)
 * @param sm SM parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void match_low_to_high_scale_model_1loop(
   ScalarSingletDM_Z3_mass_eigenstates& model,
   const Standard_model& sm,
   int idx)
{
   // tree-level ScalarSingletDM_Z3 parameters
   const auto model_0l = calculate_ScalarSingletDM_Z3_tree_level(model, sm);
   const auto sm_0l = calculate_SM_tree_level(sm, model_0l, idx);

   // 1-loop parameters
   const auto model_1l = calculate_ScalarSingletDM_Z3_1loop(sm_0l, sm, model_0l, model);
 ...
 * CP-even Higgs boson mass of the ScalarSingletDM_Z3.
 *
...
void match_high_to_low_scale_model_tree_level(
   Standard_model& sm, const ScalarSingletDM_Z3_mass_eigenstates& model, int idx)
{
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the ScalarSingletDM_Z3 at the tree level from the known Standard Model
 * couplings and the SM vev.
 */
void match_low_to_high_scale_model_tree_level(
   ScalarSingletDM_Z3_mass_eigenstates& model, const Standard_model& sm_input)
{
   auto sm = sm_input;
   sm.calculate_DRbar_masses();

   model.set_g1(sm.get_g1()*standard_model_info::normalization_g1/ScalarSingletDM_Z3_info::normalization_g1);
   model.set_g2(sm.get_g2()*standard_model_info::normalization_g2/ScalarSingletDM_Z3_info::normalization_g2);
   model.set_g3(sm.get_g3()*standard_model_info::normalization_g3/ScalarSingletDM_Z3_info::normalization_g3);
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the ScalarSingletDM_Z3 at the 1-loop level from the known Standard Model
 * couplings and the SM vev.
 *
 ...
 * determine the ScalarSingletDM_Z3 at the right loop order to avoid the
...
void match_low_to_high_scale_model(
   ScalarSingletDM_Z3_mass_eigenstates& model, const Standard_model& sm, int loop_order, int idx)
{
   if (loop_order == 0) {
 ...
 * CP-even Higgs boson mass of the ScalarSingletDM_Z3 by requiring that the
...
void match_high_to_low_scale_model(
   Standard_model& sm, const ScalarSingletDM_Z3_mass_eigenstates& model, int loop_order, int idx)
{
   if (loop_order == 0) {
