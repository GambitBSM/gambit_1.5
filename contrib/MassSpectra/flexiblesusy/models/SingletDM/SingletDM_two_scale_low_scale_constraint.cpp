...
void SingletDM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
...
double SingletDM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   SingletDM_weinberg_angle::Sm_parameters sm_pars;
   ...
   SingletDM_weinberg_angle weinberg(MODEL, sm_pars);
...
void SingletDM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   ...
   if (IsFinite(new_g1)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         SingletDM_info::g1, new_g1, get_scale());
   ...
   if (IsFinite(new_g2)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         SingletDM_info::g2, new_g2, get_scale());
...
double SingletDM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void SingletDM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("SingletDM_low_scale_constraint<Two_scale>: "
