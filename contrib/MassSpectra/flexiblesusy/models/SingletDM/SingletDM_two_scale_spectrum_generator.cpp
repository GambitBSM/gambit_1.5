...
#include "SingletDM_standard_model_two_scale_matching.hpp"
...
double SingletDM_spectrum_generator<Two_scale>::get_pole_mass_scale(double susy_scale) const
{
...
double SingletDM_spectrum_generator<Two_scale>::get_eft_pole_mass_scale(double susy_scale, double Mt) const
{
 ...
 */
void SingletDM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const SingletDM_input_parameters& input)
{
   ...
   SingletDM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<SingletDM_ewsb_solver<Two_scale> >(ewsb_solver));

   SingletDM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   SingletDM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   ...
   SingletDM_standard_model_matching_up<Two_scale> matching_up(
      &eft, &model, scale_getter, matching_loop_order_up, index);
   SingletDM_standard_model_matching_down<Two_scale> matching_down(
   ...
   // convergence tester for SingletDM
   SingletDM_convergence_tester<Two_scale> model_ct(
   ...
   SingletDM_standard_model_initial_guesser<Two_scale> initial_guesser(&model, &eft, qedqcd,
 ...
 */
void SingletDM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   SingletDM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void SingletDM_spectrum_generator<Two_scale>::calculate_spectrum(double Mt, double MW)
{
   ...
   if (eft.get_problems().is_running_tachyon(standard_model_info::hh))
      model.get_problems().flag_running_tachyon(SingletDM_info::hh);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::hh))
      model.get_problems().flag_pole_tachyon(SingletDM_info::hh);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VZ))
      model.get_problems().flag_running_tachyon(SingletDM_info::VZ);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VZ))
      model.get_problems().flag_pole_tachyon(SingletDM_info::VZ);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VWp))
      model.get_problems().flag_running_tachyon(SingletDM_info::VWp);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VWp))
      model.get_problems().flag_pole_tachyon(SingletDM_info::VWp);
}
