...
namespace SingletDM_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSSingletDMMessage", 1);
...
class SingletDM_spectrum {
public:
   virtual ~SingletDM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(SingletDM_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const SingletDM_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const SingletDM_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class SingletDM_spectrum_impl : public SingletDM_spectrum
{
public:
   virtual ~SingletDM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(SingletDM_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const SingletDM_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const SingletDM_input_parameters&) override;
...
public:
   ...
   void set_input_parameters(const SingletDM_input_parameters& input_) { input = input_; }
...
private:
   SingletDM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<SingletDM_spectrum> spectrum{nullptr};  ///< spectrum information

   SingletDM_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, SingletDM_info::particle_names[i].c_str());
      }
   }
}
...
SingletDM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const SingletDM_slha<Model>& model, MLINK link)
{
   MLPutRule(link, SingletDM_info::model_name);
...
template <typename Solver_type>
void SingletDM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void SingletDM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const SingletDM_input_parameters& input
)
{
   SingletDM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void SingletDM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void SingletDM_spectrum_impl<Solver_type>::fill_slha_io(SingletDM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   SingletDM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSSingletDMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSSingletDMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new SingletDM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSSingletDMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMGetSettings"))
...
DLLEXPORT int FSSingletDMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMOpenHandle(
...
DLLEXPORT int FSSingletDMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::SingletDM_librarylink;
...
DLLEXPORT int FSSingletDMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSingletDMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SingletDM_librarylink;

   if (!check_number_of_args(link, 1, "FSSingletDMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSSingletDMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSSingletDMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSSingletDMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
