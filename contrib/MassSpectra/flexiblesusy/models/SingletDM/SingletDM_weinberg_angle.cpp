...
#define CLASSNAME SingletDM_weinberg_angle
 ...
 */
CLASSNAME::SingletDM_weinberg_angle(
...
void CLASSNAME::set_model(const SingletDM_mass_eigenstates* model_)
{
   model = model_;
}
...
{
   const double gY = MODEL->get_g1() * SingletDM_info::normalization_g1;
   const double g2 = MODEL->get_g2() * SingletDM_info::normalization_g2;
...
std::complex<double> CLASSNAME::delta_vb_wave_Fe(int gO1) const
{
   ...
   const std::complex<double> result = SUM(gI2,0,2,-(AbsSqr(Cp(bar(Fe(gI2)),Fe(
      gO1),hh)(PL))*B1(0,Sqr(MFe(gI2)),Sqr(Mhh))));
