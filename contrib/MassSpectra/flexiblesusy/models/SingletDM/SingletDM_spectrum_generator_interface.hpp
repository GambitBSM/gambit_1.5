...
template <class T>
class SingletDM_spectrum_generator_interface {
public:
   ...
   { return model; }
   SingletDM_slha<SingletDM<T>> get_model_slha() const
   { return SingletDM_slha<SingletDM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const SingletDM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "SingletDM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(SingletDM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const SingletDM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void SingletDM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void SingletDM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const SingletDM_input_parameters& input)
{
 ...
 */
template <class T>
void SingletDM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   SingletDM_mass_eigenstates tmp_model(model);
   try {
   ...
   SingletDM_parameter_getter parameter_getter;
   Coupling_monitor<SingletDM_mass_eigenstates, SingletDM_parameter_getter>
 ...
 */
template <class T>
void SingletDM_spectrum_generator_interface<T>::translate_exception_to_problem(SingletDM<T>& model)
{
   try {
      throw;
