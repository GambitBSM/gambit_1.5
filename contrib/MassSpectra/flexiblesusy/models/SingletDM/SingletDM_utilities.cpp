...
SingletDM_spectrum_plotter::SingletDM_spectrum_plotter(const SingletDM_mass_eigenstates& model)
{
 ...
 */
void to_database(
   const std::string& file_name, const SingletDM_mass_eigenstates& model,
   ...
   const SingletDM_observables* observables)
{
   ...
   append(names, SingletDM_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, SingletDM_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, SingletDM_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, SingletDM_parameter_getter().get_DRbar_mass_names());
   append(names, SingletDM_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, SingletDM_parameter_getter().get_pole_mass_names());
   append(names, SingletDM_parameter_getter().get_pole_mixing_names());
   ...
   if (SingletDM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, SingletDM_observables::get_names());
...
SingletDM_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, SingletDM_observables* observables)
{
   using utilities::append;

   SingletDM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = SingletDM_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = SingletDM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = SingletDM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = SingletDM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? SingletDM_observables::NUMBER_OF_OBSERVABLES : 0);
