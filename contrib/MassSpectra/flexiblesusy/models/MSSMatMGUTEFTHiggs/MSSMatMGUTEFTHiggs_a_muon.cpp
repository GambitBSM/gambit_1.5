...
// File generated at Thu 10 May 2018 14:36:21

/**
 * @file MSSMatMGUTEFTHiggs_a_muon.cpp
 *
 * This file was generated at Thu 10 May 2018 14:36:21 with FlexibleSUSY
...
#include "MSSMatMGUTEFTHiggs_cxx_diagrams.hpp"
...
double get_MSUSY(const MSSMatMGUTEFTHiggs_mass_eigenstates& model)
{
...
void run_to_MSUSY(MSSMatMGUTEFTHiggs_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMatMGUTEFTHiggs_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMatMGUTEFTHiggs_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMatMGUTEFTHiggs_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMatMGUTEFTHiggs_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(MSSMatMGUTEFTHiggs_mass_eigenstates& model)
{
   VERBOSE_MSG("MSSMatMGUTEFTHiggs_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const MSSMatMGUTEFTHiggs_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("MSSMatMGUTEFTHiggs_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double MSSMatMGUTEFTHiggs_a_muon::calculate_a_muon(const MSSMatMGUTEFTHiggs_mass_eigenstates& model_)
{
   MSSMatMGUTEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMatMGUTEFTHiggs_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMatMGUTEFTHiggs_a_muon:" << e.what());
...
double MSSMatMGUTEFTHiggs_a_muon::calculate_a_muon_uncertainty(const MSSMatMGUTEFTHiggs_mass_eigenstates& model_)
{
   MSSMatMGUTEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMatMGUTEFTHiggs_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMatMGUTEFTHiggs_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
