...
#include "MSSMatMGUTEFTHiggs_utilities.hpp"
...
MSSMatMGUTEFTHiggs_spectrum_plotter::MSSMatMGUTEFTHiggs_spectrum_plotter(const MSSMatMGUTEFTHiggs_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void MSSMatMGUTEFTHiggs_spectrum_plotter::extract_spectrum(const MSSMatMGUTEFTHiggs_mass_eigenstates& model)
{
   spectrum.clear();
...
void MSSMatMGUTEFTHiggs_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MSSMatMGUTEFTHiggs_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MSSMatMGUTEFTHiggs_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MSSMatMGUTEFTHiggs_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MSSMatMGUTEFTHiggs_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MSSMatMGUTEFTHiggs_spectrum_plotter::write_spectrum: "
...
namespace MSSMatMGUTEFTHiggs_database {
 ...
 */
void to_database(
   const std::string& file_name, const MSSMatMGUTEFTHiggs_mass_eigenstates& model,
   ...
   const MSSMatMGUTEFTHiggs_observables* observables)
{
   ...
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_DRbar_mass_names());
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_pole_mass_names());
   append(names, MSSMatMGUTEFTHiggs_parameter_getter().get_pole_mixing_names());
   ...
   if (MSSMatMGUTEFTHiggs_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MSSMatMGUTEFTHiggs_observables::get_names());
 ...
 */
MSSMatMGUTEFTHiggs_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, MSSMatMGUTEFTHiggs_observables* observables)
{
   using utilities::append;

   MSSMatMGUTEFTHiggs_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = MSSMatMGUTEFTHiggs_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = MSSMatMGUTEFTHiggs_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = MSSMatMGUTEFTHiggs_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = MSSMatMGUTEFTHiggs_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? MSSMatMGUTEFTHiggs_observables::NUMBER_OF_OBSERVABLES : 0);
