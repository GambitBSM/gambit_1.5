...
#ifndef MSSMatMGUTEFTHiggs_SPECTRUM_GENERATOR_INTERFACE_H
#define MSSMatMGUTEFTHiggs_SPECTRUM_GENERATOR_INTERFACE_H

#include "MSSMatMGUTEFTHiggs_mass_eigenstates.hpp"
#include "MSSMatMGUTEFTHiggs_model.hpp"
...
template <class T>
class MSSMatMGUTEFTHiggs_spectrum_generator_interface {
public:
   virtual ~MSSMatMGUTEFTHiggs_spectrum_generator_interface() = default;

   std::tuple<MSSMatMGUTEFTHiggs<T>, standard_model::StandardModel<T>> get_models() const
   { return std::make_tuple(model, eft); }
   std::tuple<MSSMatMGUTEFTHiggs_slha<MSSMatMGUTEFTHiggs<T>>, standard_model::StandardModel<T>> get_models_slha() const
   { return std::make_tuple(MSSMatMGUTEFTHiggs_slha<MSSMatMGUTEFTHiggs<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.), eft); }

   MSSMatMGUTEFTHiggs<T> get_model() const
   { return model; }
   MSSMatMGUTEFTHiggs_slha<MSSMatMGUTEFTHiggs<T>> get_model_slha() const
   { return MSSMatMGUTEFTHiggs_slha<MSSMatMGUTEFTHiggs<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMatMGUTEFTHiggs_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMatMGUTEFTHiggs_spectrum.dat") const;

protected:
   MSSMatMGUTEFTHiggs<T> model;
   ...
   void translate_exception_to_problem(MSSMatMGUTEFTHiggs<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMatMGUTEFTHiggs_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMatMGUTEFTHiggs_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMatMGUTEFTHiggs_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMatMGUTEFTHiggs_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMatMGUTEFTHiggs_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMatMGUTEFTHiggs_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMatMGUTEFTHiggs_parameter_getter parameter_getter;
   Coupling_monitor<MSSMatMGUTEFTHiggs_mass_eigenstates, MSSMatMGUTEFTHiggs_parameter_getter>
 ...
 */
template <class T>
void MSSMatMGUTEFTHiggs_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMatMGUTEFTHiggs_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMatMGUTEFTHiggs_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMatMGUTEFTHiggs<T>& model)
{
   try {
      throw;
