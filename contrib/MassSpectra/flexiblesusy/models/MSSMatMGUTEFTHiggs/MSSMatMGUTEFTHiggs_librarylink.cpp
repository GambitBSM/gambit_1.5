...
namespace MSSMatMGUTEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMatMGUTEFTHiggsMessage", 1);
...
class MSSMatMGUTEFTHiggs_spectrum {
public:
   virtual ~MSSMatMGUTEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMatMGUTEFTHiggs_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMatMGUTEFTHiggs_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMGUTEFTHiggs_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSMatMGUTEFTHiggs_spectrum_impl : public MSSMatMGUTEFTHiggs_spectrum
{
public:
   virtual ~MSSMatMGUTEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMatMGUTEFTHiggs_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMatMGUTEFTHiggs_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMatMGUTEFTHiggs_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSMatMGUTEFTHiggs_slha<MSSMatMGUTEFTHiggs<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMatMGUTEFTHiggs_scales scales{};              ///< scale information
   MSSMatMGUTEFTHiggs_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSMatMGUTEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   MSSMatMGUTEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMatMGUTEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMatMGUTEFTHiggs_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSMatMGUTEFTHiggs_info::particle_names[i].c_str());
      }
   }
}
...
MSSMatMGUTEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSMatMGUTEFTHiggs_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSMatMGUTEFTHiggs_info::model_name);
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMatMGUTEFTHiggs_input_parameters& input
)
{
   MSSMatMGUTEFTHiggs_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMatMGUTEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(MSSMatMGUTEFTHiggs_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSMatMGUTEFTHiggs_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMatMGUTEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMatMGUTEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMatMGUTEFTHiggs_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsGetSettings"))
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsOpenHandle(
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMatMGUTEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMatMGUTEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMatMGUTEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMatMGUTEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMatMGUTEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMatMGUTEFTHiggsCalculateObservables", "error", e.what());
      put_error_output(link);
   }
