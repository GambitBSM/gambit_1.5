...
SSM_initial_guesser<Two_scale>::SSM_initial_guesser(
   SSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const SSM_low_scale_constraint<Two_scale>& low_constraint_,
   const SSM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const SSM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("SSM_initial_guesser: Error: pointer to model"
                       " SSM<Two_scale> must not be zero");
}
 ...
 */
void SSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void SSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void SSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * corrections).
 */
void SSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void SSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void SSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
 ...
 */
void SSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
