...
// File generated at Wed 25 Oct 2017 18:11:27
...
#define CLASSNAME SSM_susy_parameters
#define INPUT(parameter) input.parameter
#define TRACE_STRUCT susy_traces
#define TRACE_STRUCT_TYPE Susy_traces
#define CALCULATE_TRACES(l) calc_susy_traces(l);

const int SSM_susy_parameters::numberOfParameters;

SSM_susy_parameters::SSM_susy_parameters(const SSM_input_parameters& input_)
   : input(input_)
{
   set_number_of_parameters(numberOfParameters);
}

SSM_susy_parameters::SSM_susy_parameters(
   double scale_, int loops_, int thresholds_,
   const SSM_input_parameters& input_
   , double g1_, double g2_, double g3_, double LambdaS_, double K2_, double
   Lambdax_, const Eigen::Matrix<double,3,3>& Yu_, const Eigen::Matrix<double,3
   ,3>& Yd_, const Eigen::Matrix<double,3,3>& Ye_

)
   : Beta_function()
   , g1(g1_), g2(g2_), g3(g3_), LambdaS(LambdaS_), K2(K2_), Lambdax(Lambdax_),
...
Eigen::ArrayXd SSM_susy_parameters::beta() const
{
   return calc_beta().get().unaryExpr(Chop<double>(get_zero_threshold()));
}

SSM_susy_parameters SSM_susy_parameters::calc_beta(int loops) const
{
   double beta_g1 = 0.;
   double beta_g2 = 0.;
   double beta_g3 = 0.;
   double beta_LambdaS = 0.;
   double beta_K2 = 0.;
   ...
   if (loops > 0) {
      ...
      beta_LambdaS += calc_beta_LambdaS_1_loop(TRACE_STRUCT);
      beta_K2 += calc_beta_K2_1_loop(TRACE_STRUCT);
      ...
      if (loops > 1) {
         ...
         beta_LambdaS += calc_beta_LambdaS_2_loop(TRACE_STRUCT);
         beta_K2 += calc_beta_K2_2_loop(TRACE_STRUCT);
   ...
   return SSM_susy_parameters(get_scale(), loops, get_thresholds(), input,
                    beta_g1, beta_g2, beta_g3, beta_LambdaS, beta_K2, beta_Lambdax, beta_Yu, beta_Yd, beta_Ye);
}

SSM_susy_parameters SSM_susy_parameters::calc_beta() const
{
   return calc_beta(get_loops());
}

void SSM_susy_parameters::clear()
{
   reset();
   g1 = 0.;
   g2 = 0.;
...
Eigen::ArrayXd SSM_susy_parameters::get() const
{
...
void SSM_susy_parameters::print(std::ostream& ostr) const
{
   ostr << "susy parameters at Q = " << get_scale() << ":\n";
   ostr << "g1 = " << g1 << '\n';
   ostr << "g2 = " << g2 << '\n';
   ostr << "g3 = " << g3 << '\n';
   ostr << "LambdaS = " << LambdaS << '\n';
   ostr << "K2 = " << K2 << '\n';
...
void SSM_susy_parameters::set(const Eigen::ArrayXd& pars)
{
   g1 = pars(0);
   g2 = pars(1);
   g3 = pars(2);
...
const SSM_input_parameters& SSM_susy_parameters::get_input() const
{
   return input;
}

SSM_input_parameters& SSM_susy_parameters::get_input()
{
   return input;
}

void SSM_susy_parameters::set_input_parameters(const SSM_input_parameters& input_)
{
   input = input_;
}

SSM_susy_parameters::Susy_traces SSM_susy_parameters::calc_susy_traces(int loops) const
{
...
std::ostream& operator<<(std::ostream& ostr, const SSM_susy_parameters& susy_pars)
{
