...
double SSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void SSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const SSM_input_parameters& input)
{
   ...
   SSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<SSM_ewsb_solver<Two_scale> >(ewsb_solver));

   SSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   SSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   SSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   SSM_convergence_tester<Two_scale> convergence_tester(
   ...
   SSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void SSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   SSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void SSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
