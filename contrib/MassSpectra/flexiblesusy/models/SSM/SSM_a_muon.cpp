...
// File generated at Thu 12 Oct 2017 14:09:05

/**
 * @file SSM_a_muon.cpp
 *
 * This file was generated at Thu 12 Oct 2017 14:09:05 with FlexibleSUSY
...
double get_MSUSY(const SSM_mass_eigenstates& model)
{
   return 0.;

}

void run_to_MSUSY(SSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("SSM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("SSM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("SSM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("SSM_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(SSM_mass_eigenstates& model)
{
   VERBOSE_MSG("SSM_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const SSM_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("SSM_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double SSM_a_muon::calculate_a_muon(const SSM_mass_eigenstates& model_)
{
   SSM_mass_eigenstates model(model_);

   VERBOSE_MSG("SSM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SSM_a_muon:" << e.what());
...
double SSM_a_muon::calculate_a_muon_uncertainty(const SSM_mass_eigenstates& model_)
{
   SSM_mass_eigenstates model(model_);

   VERBOSE_MSG("SSM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SSM_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
