...
// File generated at Thu 12 Oct 2017 14:09:04

/**
 * @file SSM_two_scale_ewsb_solver.cpp
 *
 * @brief implementation of EWSB solver for two-scale iteration
 *
 * This file was generated at Thu 12 Oct 2017 14:09:04 with FlexibleSUSY
...
int CLASSNAME::solve_iteratively_with(
   SSM_mass_eigenstates& model_to_solve, EWSB_solver* solver, const EWSB_vector_t& x_init)
{
 ...
 * @param solver solver
 */
void CLASSNAME::set_ewsb_solution(SSM_mass_eigenstates& model, const EWSB_solver* solver)
{
   const auto solution = solver->get_solution();

   const double mu2 = solution(0);
   const double MS = solution(1);
   model.set_mu2(mu2);
   model.set_MS(MS);
...
void CLASSNAME::set_best_ewsb_solution(SSM_mass_eigenstates& model, It first, It last)
{
...
int CLASSNAME::solve_iteratively_at(SSM_mass_eigenstates& model_to_solve, int l)
{
...
int CLASSNAME::solve(SSM_mass_eigenstates& model_to_solve)
{
   if (loop_order == 0) {
...
int CLASSNAME::solve_tree_level(SSM_mass_eigenstates& model)
{
   int error = EWSB_solver::SUCCESS;

   const auto v = MODELPARAMETER(v);
   const auto K1 = MODELPARAMETER(K1);
   ...
   mu2 = Re((0.5*(-2*K1*v*vS - Cube(v)*Lambdax - K2*v*Sqr(vS)))/v);
   MS = Re((0.5*(-4*LambdaS*Cube(vS) - K1*Sqr(v) - K2*vS*Sqr(v) - 2*Kappa*Sqr(
      vS)))/vS);

   const bool is_finite = IsFinite(mu2) && IsFinite(MS);
...
CLASSNAME::EWSB_vector_t CLASSNAME::initial_guess(const SSM_mass_eigenstates& model) const
{
...
CLASSNAME::EWSB_vector_t CLASSNAME::tadpole_equations(const SSM_mass_eigenstates& model) const
{
 ...
 */
CLASSNAME::EWSB_vector_t CLASSNAME::ewsb_step(const SSM_mass_eigenstates& model) const
{
   ...
   mu2 = Re((0.5*(-2*K1*v*vS - Cube(v)*Lambdax + 2*tadpole[0] - K2*v*Sqr(vS)))
      /v);
   MS = Re((0.5*(-4*LambdaS*Cube(vS) + 2*tadpole[1] - K1*Sqr(v) - K2*vS*Sqr(v)
      - 2*Kappa*Sqr(vS)))/vS);
