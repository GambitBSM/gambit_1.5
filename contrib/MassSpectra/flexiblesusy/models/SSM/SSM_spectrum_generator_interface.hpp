...
// File generated at Thu 12 Oct 2017 14:09:01

#ifndef SSM_SPECTRUM_GENERATOR_INTERFACE_H
#define SSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class SSM_spectrum_generator_interface {
public:
   virtual ~SSM_spectrum_generator_interface() = default;

   std::tuple<SSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<SSM_slha<SSM<T>>> get_models_slha() const
   { return std::make_tuple(SSM_slha<SSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   SSM<T> get_model() const
   { return model; }
   SSM_slha<SSM<T>> get_model_slha() const
   { return SSM_slha<SSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const SSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "SSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(SSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const SSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void SSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void SSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const SSM_input_parameters& input)
{
 ...
 */
template <class T>
void SSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   SSM_mass_eigenstates tmp_model(model);
   try {
   ...
   SSM_parameter_getter parameter_getter;
   Coupling_monitor<SSM_mass_eigenstates, SSM_parameter_getter>
 ...
 */
template <class T>
void SSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   SSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void SSM_spectrum_generator_interface<T>::translate_exception_to_problem(SSM<T>& model)
{
   try {
      throw;
