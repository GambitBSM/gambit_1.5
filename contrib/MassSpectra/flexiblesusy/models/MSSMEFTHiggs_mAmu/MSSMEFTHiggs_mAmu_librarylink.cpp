...
// File generated at Thu 10 May 2018 14:40:47

#include "config.h"

#include "MSSMEFTHiggs_mAmu_info.hpp"
#include "MSSMEFTHiggs_mAmu_input_parameters.hpp"
#include "MSSMEFTHiggs_mAmu_observables.hpp"
#include "MSSMEFTHiggs_mAmu_physical.hpp"
#include "MSSMEFTHiggs_mAmu_slha_io.hpp"
#include "MSSMEFTHiggs_mAmu_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "MSSMEFTHiggs_mAmu_two_scale_model.hpp"
#include "MSSMEFTHiggs_mAmu_two_scale_spectrum_generator.hpp"
#endif
...
namespace MSSMEFTHiggs_mAmu_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMEFTHiggs_mAmuMessage", 1);
...
class MSSMEFTHiggs_mAmu_spectrum {
public:
   virtual ~MSSMEFTHiggs_mAmu_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMEFTHiggs_mAmu_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMEFTHiggs_mAmu_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMEFTHiggs_mAmu_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class MSSMEFTHiggs_mAmu_spectrum_impl : public MSSMEFTHiggs_mAmu_spectrum
{
public:
   virtual ~MSSMEFTHiggs_mAmu_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMEFTHiggs_mAmu_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMEFTHiggs_mAmu_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMEFTHiggs_mAmu_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<MSSMEFTHiggs_mAmu_slha<MSSMEFTHiggs_mAmu<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMEFTHiggs_mAmu_scales scales{};              ///< scale information
   MSSMEFTHiggs_mAmu_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const MSSMEFTHiggs_mAmu_input_parameters& input_) { input = input_; }
...
private:
   MSSMEFTHiggs_mAmu_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMEFTHiggs_mAmu_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMEFTHiggs_mAmu_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MSSMEFTHiggs_mAmu_info::particle_names[i].c_str());
      }
   }
}
...
MSSMEFTHiggs_mAmu_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSMEFTHiggs_mAmu_slha_io slha_io;
...
void put_spectrum(const MSSMEFTHiggs_mAmu_slha<Model>& model, MLINK link)
{
   MLPutRule(link, MSSMEFTHiggs_mAmu_info::model_name);
...
template <typename Solver_type>
void MSSMEFTHiggs_mAmu_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMEFTHiggs_mAmu_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMEFTHiggs_mAmu_input_parameters& input
)
{
   MSSMEFTHiggs_mAmu_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMEFTHiggs_mAmu_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMEFTHiggs_mAmu_spectrum_impl<Solver_type>::fill_slha_io(MSSMEFTHiggs_mAmu_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MSSMEFTHiggs_mAmu_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMEFTHiggs_mAmuCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMEFTHiggs_mAmuCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMEFTHiggs_mAmu_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuGetSettings"))
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuOpenHandle(
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggs_mAmuCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_mAmu_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggs_mAmuCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMEFTHiggs_mAmuCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMEFTHiggs_mAmuCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMEFTHiggs_mAmuCalculateObservables", "error", e.what());
      put_error_output(link);
   }
