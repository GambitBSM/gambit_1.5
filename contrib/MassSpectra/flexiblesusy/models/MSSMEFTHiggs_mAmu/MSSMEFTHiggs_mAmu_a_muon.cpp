...
/**
 * @file MSSMEFTHiggs_mAmu_a_muon.cpp
 *
 * This file was generated at Thu 10 May 2018 14:40:38 with FlexibleSUSY
 * 2.0.1 and SARAH 4.12.2 .
 */

#include "MSSMEFTHiggs_mAmu_a_muon.hpp"
#include "MSSMEFTHiggs_mAmu_mass_eigenstates.hpp"

#include "MSSMEFTHiggs_mAmu_cxx_diagrams.hpp"
...
double get_MSUSY(const MSSMEFTHiggs_mAmu_mass_eigenstates& model)
{
...
void run_to_MSUSY(MSSMEFTHiggs_mAmu_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMEFTHiggs_mAmu_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMEFTHiggs_mAmu_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMEFTHiggs_mAmu_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMEFTHiggs_mAmu_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(MSSMEFTHiggs_mAmu_mass_eigenstates& model)
{
   VERBOSE_MSG("MSSMEFTHiggs_mAmu_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const MSSMEFTHiggs_mAmu_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("MSSMEFTHiggs_mAmu_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double MSSMEFTHiggs_mAmu_a_muon::calculate_a_muon(const MSSMEFTHiggs_mAmu_mass_eigenstates& model_)
{
   MSSMEFTHiggs_mAmu_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMEFTHiggs_mAmu_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMEFTHiggs_mAmu_a_muon:" << e.what());
...
double MSSMEFTHiggs_mAmu_a_muon::calculate_a_muon_uncertainty(const MSSMEFTHiggs_mAmu_mass_eigenstates& model_)
{
   MSSMEFTHiggs_mAmu_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMEFTHiggs_mAmu_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMEFTHiggs_mAmu_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
