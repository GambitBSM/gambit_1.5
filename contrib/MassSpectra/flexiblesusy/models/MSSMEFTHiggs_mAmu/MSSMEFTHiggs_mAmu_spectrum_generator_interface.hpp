...
#ifndef MSSMEFTHiggs_mAmu_SPECTRUM_GENERATOR_INTERFACE_H
#define MSSMEFTHiggs_mAmu_SPECTRUM_GENERATOR_INTERFACE_H

#include "MSSMEFTHiggs_mAmu_mass_eigenstates.hpp"
#include "MSSMEFTHiggs_mAmu_model.hpp"
...
template <class T>
class MSSMEFTHiggs_mAmu_spectrum_generator_interface {
public:
   virtual ~MSSMEFTHiggs_mAmu_spectrum_generator_interface() = default;

   std::tuple<MSSMEFTHiggs_mAmu<T>, standard_model::StandardModel<T>> get_models() const
   { return std::make_tuple(model, eft); }
   std::tuple<MSSMEFTHiggs_mAmu_slha<MSSMEFTHiggs_mAmu<T>>, standard_model::StandardModel<T>> get_models_slha() const
   { return std::make_tuple(MSSMEFTHiggs_mAmu_slha<MSSMEFTHiggs_mAmu<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.), eft); }

   MSSMEFTHiggs_mAmu<T> get_model() const
   { return model; }
   MSSMEFTHiggs_mAmu_slha<MSSMEFTHiggs_mAmu<T>> get_model_slha() const
   { return MSSMEFTHiggs_mAmu_slha<MSSMEFTHiggs_mAmu<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMEFTHiggs_mAmu_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMEFTHiggs_mAmu_spectrum.dat") const;

protected:
   MSSMEFTHiggs_mAmu<T> model;
   ...
   void translate_exception_to_problem(MSSMEFTHiggs_mAmu<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMEFTHiggs_mAmu_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMEFTHiggs_mAmu_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMEFTHiggs_mAmu_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMEFTHiggs_mAmu_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMEFTHiggs_mAmu_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMEFTHiggs_mAmu_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMEFTHiggs_mAmu_parameter_getter parameter_getter;
   Coupling_monitor<MSSMEFTHiggs_mAmu_mass_eigenstates, MSSMEFTHiggs_mAmu_parameter_getter>
 ...
 */
template <class T>
void MSSMEFTHiggs_mAmu_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMEFTHiggs_mAmu_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMEFTHiggs_mAmu_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMEFTHiggs_mAmu<T>& model)
{
   try {
      throw;
