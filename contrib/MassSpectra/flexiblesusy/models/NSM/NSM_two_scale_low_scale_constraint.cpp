...
#include "NSM_weinberg_angle.hpp"
...
NSM_low_scale_constraint<Two_scale>::NSM_low_scale_constraint(
   NSM<Two_scale>* model_, const softsusy::QedQcd& qedqcd_)
   : model(model_)
   , qedqcd(qedqcd_)
{
   initialize();
...
void NSM_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
   ...
   MODEL->set_vH(Re((2*MZMSbar)/Sqrt(Sqr(g1) + Sqr(g2))));
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
   MODEL->set_g1(new_g1);
...
const Eigen::Matrix<std::complex<double>,3,3>& NSM_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& NSM_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double NSM_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double NSM_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void NSM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<NSM<Two_scale>*>(model_);
}

void NSM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& NSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void NSM_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void NSM_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void NSM_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void NSM_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double NSM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();
...
void NSM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
   ...
   if (IsFinite(new_g2)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         NSM_info::g2, new_g2, get_scale());
      new_g2 = Electroweak_constants::g2;
   }

}

double NSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double NSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double NSM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void NSM_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void NSM_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void NSM_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void NSM_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void NSM_low_scale_constraint<Two_scale>::calculate_MNeutrino_DRbar()
{
   neutrinoDRbar.setZero();
...
void NSM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("NSM_low_scale_constraint<Two_scale>: "
