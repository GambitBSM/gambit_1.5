...
// File generated at Sun 24 Sep 2017 15:56:18
...
#include "NSM_two_scale_ewsb_solver.hpp"
#endif
...
CLASSNAME::NSM_mass_eigenstates(const NSM_input_parameters& input_)
   ...
   , ewsb_solver(new NSM_ewsb_solver<Two_scale>())
#endif
{
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<NSM_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("NSM_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("NSM_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("NSM_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving NSM EWSB at " << ewsb_loop_order << "-loop order");
...
{
   const auto save_mH2_raii = make_raii_save(mH2);
...
{
   if (PHYSICAL(Mhh).tail<2>().minCoeff() < 0.) problems.flag_pole_tachyon(NSM_info::hh);
...
void CLASSNAME::calculate_MHp()
{
   ...
   if (MHp < 0.) {
      problems.flag_running_tachyon(NSM_info::Hp);
   }
...
{
   const double mass_matrix_Ah = Re(0.25*(-4*(mH2 - vS*(vS*Lambda3 +
      Lambda4) - Lambda1*Sqr(vH)) + Sqr(vH)*Sqr(g2*Cos(ThetaW()) + g1*Sin(
      ThetaW()))));
...
void CLASSNAME::calculate_MAh()
{
   ...
   if (MAh < 0.) {
      problems.flag_running_tachyon(NSM_info::Ah);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(NSM_info::hh);
   }
...
void CLASSNAME::calculate_MVWp()
{
   ...
   if (MVWp < 0.) {
      problems.flag_running_tachyon(NSM_info::VWp);
   }
...
Eigen::Matrix<double,2,2> CLASSNAME::get_mass_matrix_VPVZ() const
{
   ...
   mass_matrix_VPVZ(0,0) = 0.25*Sqr(g1)*Sqr(vH);
   mass_matrix_VPVZ(0,1) = -0.25*g1*g2*Sqr(vH);
   mass_matrix_VPVZ(1,1) = 0.25*Sqr(g2)*Sqr(vH);
...
{
   double result = Re(-(mH2*vH) + Cube(vH)*Lambda1 + vH*vS*Lambda4 + vH*Lambda3
      *Sqr(vS));
...
{
   double result = Re(-2*mS2*vS + 4*Cube(vS)*Lambda2 + vS*Lambda3*Sqr(vH) + 0.5
      *Lambda4*Sqr(vH) + 3*Lambda5*Sqr(vS));
...
{
   const std::complex<double> result = -2*vH*KroneckerDelta(0,gO2)*Lambda1 -
      KroneckerDelta(1,gO2)*(2*vS*Lambda3 + Lambda4);
...
{
   const double result = -0.25*vH*KroneckerDelta(0,gO1)*Sqr(g2);
...
{
   const double result = -0.25*vH*KroneckerDelta(0,gO1)*Sqr(g2*Cos(ThetaW()) +
      g1*Sin(ThetaW()));
...
{
   const double result = 0.5*vH*KroneckerDelta(0,gO2)*Sqr(g2*Cos(ThetaW()) + g1
      *Sin(ThetaW()));
...
{
   const double result = 0.5*vH*KroneckerDelta(0,gO2)*Sqr(g2);
...
{
   const std::complex<double> result = -2*(KroneckerDelta(0,gO1)*KroneckerDelta
      (0,gO2)*Lambda1 + KroneckerDelta(1,gO1)*KroneckerDelta(1,gO2)*Lambda3);
...
std::complex<double> CLASSNAME::CpAhUhhVZ(int gO2) const
{
   const std::complex<double> result = std::complex<double>(0,-0.5)*
      KroneckerDelta(0,gO2)*(g2*Cos(ThetaW()) + g1*Sin(ThetaW()));
...
{
   const std::complex<double> result = 0.5*KroneckerDelta(0,gO1)*KroneckerDelta
      (0,gO2)*Sqr(g2*Cos(ThetaW()) + g1*Sin(ThetaW()));
...
{
   const std::complex<double> result = -2*(Conj(ZH(gI1,1))*(Conj(ZH(gI2,0))*(
      KroneckerDelta(0,gO2)*KroneckerDelta(1,gO1) + KroneckerDelta(0,gO1)*
      KroneckerDelta(1,gO2))*Lambda3 + Conj(ZH(gI2,1))*(12*KroneckerDelta(1,gO1)*
      KroneckerDelta(1,gO2)*Lambda2 + KroneckerDelta(0,gO1)*KroneckerDelta(0,gO2)*
      Lambda3)) + Conj(ZH(gI1,0))*(Conj(ZH(gI2,1))*(KroneckerDelta(0,gO2)*
      KroneckerDelta(1,gO1) + KroneckerDelta(0,gO1)*KroneckerDelta(1,gO2))*Lambda3
      + Conj(ZH(gI2,0))*(3*KroneckerDelta(0,gO1)*KroneckerDelta(0,gO2)*Lambda1 +
...
{
   const std::complex<double> result = -(Conj(ZH(gI1,0))*(Conj(ZH(gI2,1))*(2*vH
      *KroneckerDelta(1,gO2)*Lambda3 + KroneckerDelta(0,gO2)*(2*vS*Lambda3 +
      Lambda4)) + Conj(ZH(gI2,0))*(6*vH*KroneckerDelta(0,gO2)*Lambda1 +
      KroneckerDelta(1,gO2)*(2*vS*Lambda3 + Lambda4)))) - Conj(ZH(gI1,1))*(Conj(ZH
      (gI2,0))*(2*vH*KroneckerDelta(1,gO2)*Lambda3 + KroneckerDelta(0,gO2)*(2*vS*
      Lambda3 + Lambda4)) + 2*Conj(ZH(gI2,1))*(vH*KroneckerDelta(0,gO2)*Lambda3 +
      3*KroneckerDelta(1,gO2)*(4*vS*Lambda2 + Lambda5)));
...
{
   const double result = -0.25*g2*vH*(g2*Cos(ThetaW()) - g1*Sin(ThetaW()));
...
{
   const double result = 0.5*g1*g2*vH*Cos(ThetaW());
...
{
   const double result = -0.5*g1*g2*vH*Sin(ThetaW());
...
{
   const double result = 0.5*(g1*Cos(ThetaW()) + g2*Sin(ThetaW()));
...
{
   const double result = 0.5*(g2*Cos(ThetaW()) - g1*Sin(ThetaW()));
...
{
   const std::complex<double> result = -2*(Conj(ZH(gI1,0))*Conj(ZH(gI2,0))*
      Lambda1 + Conj(ZH(gI1,1))*Conj(ZH(gI2,1))*Lambda3);
...
{
   const std::complex<double> result = -2*vH*Conj(ZH(gI2,0))*Lambda1 - Conj(ZH(
      gI2,1))*(2*vS*Lambda3 + Lambda4);
...
{
   const std::complex<double> result = 0.5*g2*Conj(ZH(gI2,0));
...
{
   const std::complex<double> result = std::complex<double>(0,-0.25)*vH*Sqr(g2)
      ;
...
{
   const std::complex<double> result = -2*vH*Conj(ZH(gI1,0))*Lambda1 - Conj(ZH(
      gI1,1))*(2*vS*Lambda3 + Lambda4);
...
std::complex<double> CLASSNAME::CpbarFdFdAhPR(int gI1, int gI2) const
{
   const std::complex<double> result = std::complex<double>(0.,
      0.7071067811865475)*SUM(j2,0,2,SUM(j1,0,2,Conj(Yd(j1,j2))*Ud(gI2,j1))*Vd(gI1
      ,j2));
...
{
   const std::complex<double> result = std::complex<double>(0.,
      0.7071067811865475)*SUM(j2,0,2,SUM(j1,0,2,Conj(Ye(j1,j2))*Ue(gI2,j1))*Ve(gI1
      ,j2));
...
{
   const std::complex<double> result = std::complex<double>(0,-0.5)*Conj(ZH(gI2
      ,0))*(g2*Cos(ThetaW()) + g1*Sin(ThetaW()));
...
{
   const std::complex<double> result = 0.5*Conj(ZH(gI1,0))*Conj(ZH(gI2,0))*Sqr(
      g2*Cos(ThetaW()) + g1*Sin(ThetaW()));
...
{
   const std::complex<double> result = 0.5*vH*Conj(ZH(gI2,0))*Sqr(g2*Cos(ThetaW
      ()) + g1*Sin(ThetaW()));
...
std::complex<double> CLASSNAME::CpbarUFdFdhhPL(int gO2, int gI2, int gI1) const
{
   const std::complex<double> result = IF(gO2 < 3,0.7071067811865475*Conj(ZH(
      gI1,0))*SUM(j2,0,2,Conj(Vd(gI2,j2))*Yd(gO2,j2)),0);
...
std::complex<double> CLASSNAME::CpbarUFdFdhhPR(int gO1, int gI2, int gI1) const
{
   const std::complex<double> result = IF(gO1 < 3,0.7071067811865475*Conj(ZH(
      gI1,0))*SUM(j1,0,2,Conj(Yd(j1,gO1))*Ud(gI2,j1)),0);
...
{
   const std::complex<double> result = IF(gO2 < 3,-0.7071067811865475*Conj(ZH(
      gI1,0))*SUM(j2,0,2,Conj(Vu(gI2,j2))*Yu(gO2,j2)),0);
...
{
   const std::complex<double> result = IF(gO1 < 3,-0.7071067811865475*Conj(ZH(
      gI1,0))*SUM(j1,0,2,Conj(Yu(j1,gO1))*Uu(gI2,j1)),0);
...
{
   const std::complex<double> result = IF(gO2 < 3,0.7071067811865475*Conj(ZH(
      gI1,0))*SUM(j2,0,2,Conj(Ve(gI2,j2))*Ye(gO2,j2)),0);
...
{
   const std::complex<double> result = IF(gO1 < 3,0.7071067811865475*Conj(ZH(
      gI1,0))*SUM(j1,0,2,Conj(Ye(j1,gO1))*Ue(gI2,j1)),0);
...
{
   const std::complex<double> result = 0.7071067811865475*Conj(ZH(gI1,0))*SUM(
      j2,0,2,SUM(j1,0,2,Conj(Yd(j1,j2))*Ud(gI2,j1))*Vd(gO1,j2));
...
{
   const std::complex<double> result = 0.7071067811865475*Conj(ZH(gI1,0))*SUM(
      j2,0,2,SUM(j1,0,2,Conj(Ye(j1,j2))*Ue(gI2,j1))*Ve(gO1,j2));
...
{
   const std::complex<double> result = -0.7071067811865475*Conj(ZH(gI1,0))*SUM(
      j2,0,2,SUM(j1,0,2,Conj(Yu(j1,j2))*Uu(gI2,j1))*Vu(gO1,j2));
...
{
   if (!force_output && problems.is_running_tachyon(NSM_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(NSM_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(NSM_info::hh))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(NSM_info::VWp))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(NSM_info::VWp);
