...
/**
 * @file NSM_two_scale_ewsb_solver.cpp
 *
...
int CLASSNAME::solve_iteratively(NSM_mass_eigenstates& model_to_solve)
{
   ...
   auto ewsb_stepper = [this, model](const EWSB_vector_t& ewsb_pars) mutable -> EWSB_vector_t {
      const double mH2 = ewsb_pars(0);
      const double mS2 = ewsb_pars(1);
...
int CLASSNAME::solve_iteratively_with(
   NSM_mass_eigenstates& model_to_solve, EWSB_solver* solver, const EWSB_vector_t& x_init)
{
 ...
 * @param solver solver
 */
void CLASSNAME::set_ewsb_solution(NSM_mass_eigenstates& model, const EWSB_solver* solver)
{
   const auto solution = solver->get_solution();

   const double mH2 = solution(0);
   const double mS2 = solution(1);
   model.set_mH2(mH2);
   model.set_mS2(mS2);
...
void CLASSNAME::set_best_ewsb_solution(NSM_mass_eigenstates& model, It first, It last)
{
...
int CLASSNAME::solve_iteratively_at(NSM_mass_eigenstates& model_to_solve, int l)
{
...
int CLASSNAME::solve(NSM_mass_eigenstates& model_to_solve)
{
   if (loop_order == 0) {
...
int CLASSNAME::solve_tree_level(NSM_mass_eigenstates& model)
{
   ...
   mH2 = Re((Cube(vH)*Lambda1 + vH*vS*Lambda4 + vH*Lambda3*Sqr(vS))/vH);
   mS2 = Re((0.25*(8*Cube(vS)*Lambda2 + 2*vS*Lambda3*Sqr(vH) + Lambda4*Sqr(vH)
      + 6*Lambda5*Sqr(vS)))/vS);
...
CLASSNAME::EWSB_vector_t CLASSNAME::initial_guess(const NSM_mass_eigenstates& model) const
{
   EWSB_vector_t x_init(EWSB_vector_t::Zero());

   const auto mH2 = MODELPARAMETER(mH2);
...
CLASSNAME::EWSB_vector_t CLASSNAME::tadpole_equations(const NSM_mass_eigenstates& model) const
{
 ...
 */
CLASSNAME::EWSB_vector_t CLASSNAME::ewsb_step(const NSM_mass_eigenstates& model) const
{
   ...
   mH2 = Re((Cube(vH)*Lambda1 + vH*vS*Lambda4 - tadpole[0] + vH*Lambda3*Sqr(vS)
      )/vH);
   mS2 = Re((0.25*(8*Cube(vS)*Lambda2 - 2*tadpole[1] + 2*vS*Lambda3*Sqr(vH) +
