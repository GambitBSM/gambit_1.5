...
double NSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void NSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const NSM_input_parameters& input)
{
   ...
   NSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<NSM_ewsb_solver<Two_scale> >(ewsb_solver));

   NSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   NSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   NSM_convergence_tester<Two_scale> convergence_tester(
   ...
   NSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void NSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   NSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void NSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
