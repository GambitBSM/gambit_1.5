...
template <class T>
class NSM_spectrum_generator_interface {
public:
   virtual ~NSM_spectrum_generator_interface() = default;

   std::tuple<NSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<NSM_slha<NSM<T>>> get_models_slha() const
   { return std::make_tuple(NSM_slha<NSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   NSM<T> get_model() const
   { return model; }
   NSM_slha<NSM<T>> get_model_slha() const
   { return NSM_slha<NSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const NSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(NSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const NSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void NSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const NSM_input_parameters& input)
{
 ...
 */
template <class T>
void NSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NSM_mass_eigenstates tmp_model(model);
   try {
   ...
   Coupling_monitor<NSM_mass_eigenstates, NSM_parameter_getter>
 ...
 */
template <class T>
void NSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   NSM_spectrum_plotter plotter;
 ...
 */
template <class T>
void NSM_spectrum_generator_interface<T>::translate_exception_to_problem(NSM<T>& model)
{
   try {
      throw;
