...
// File generated at Sun 24 Sep 2017 15:56:24

/**
 * @file NSM_a_muon.cpp
 *
 * This file was generated at Sun 24 Sep 2017 15:56:24 with FlexibleSUSY
 * 2.0.0-dev and SARAH 4.11.0 .
 */

#include "NSM_a_muon.hpp"
#include "NSM_mass_eigenstates.hpp"

#include "NSM_cxx_diagrams.hpp"
...
double get_MSUSY(const NSM_mass_eigenstates& model)
{
   return 0.;

}

void run_to_MSUSY(NSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NSM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NSM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NSM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NSM_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(NSM_mass_eigenstates& model)
{
   VERBOSE_MSG("NSM_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const NSM_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("NSM_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double NSM_a_muon::calculate_a_muon(const NSM_mass_eigenstates& model_)
{
   NSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NSM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NSM_a_muon:" << e.what());
...
double NSM_a_muon::calculate_a_muon_uncertainty(const NSM_mass_eigenstates& model_)
{
   NSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NSM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NSM_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
