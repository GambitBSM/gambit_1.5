...
void NSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   ...
   VERBOSE_MSG("NSM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("NSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("NSM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void NSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("NSM_spectrum_plotter::write_spectrum: "
...
std::valarray<double> NSM_spectrum_plotter::to_valarray(double v)
{
   return std::valarray<double>(&v, 1);
}

namespace NSM_database {
 ...
 */
void to_database(
   const std::string& file_name, const NSM_mass_eigenstates& model,
   ...
   const NSM_observables* observables)
{
   ...
   append(names, NSM_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, NSM_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, NSM_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, NSM_parameter_getter().get_DRbar_mass_names());
   append(names, NSM_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, NSM_parameter_getter().get_pole_mass_names());
   append(names, NSM_parameter_getter().get_pole_mixing_names());
   ...
   if (NSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, NSM_observables::get_names());
 ...
 */
NSM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, NSM_observables* observables)
{
   using utilities::append;

   NSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = NSM_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = NSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = NSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = NSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? NSM_observables::NUMBER_OF_OBSERVABLES : 0);
