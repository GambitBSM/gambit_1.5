...
const int NSM_observables::NUMBER_OF_OBSERVABLES;

NSM_observables::NSM_observables()
   : a_muon(0)
...
Eigen::ArrayXd NSM_observables::get() const
{
   Eigen::ArrayXd vec(NSM_observables::NUMBER_OF_OBSERVABLES);
...
std::vector<std::string> NSM_observables::get_names()
{
   std::vector<std::string> names(NSM_observables::NUMBER_OF_OBSERVABLES);
...
void NSM_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == NSM_observables::NUMBER_OF_OBSERVABLES);
...
NSM_observables calculate_observables(const NSM_mass_eigenstates& model,
...
NSM_observables calculate_observables(const NSM_mass_eigenstates& model,
                                              ...
                                              const Physical_input& physical_input)
{
   NSM_observables observables;

   NSM_effective_couplings effective_couplings(model, qedqcd, physical_input);
   effective_couplings.calculate_effective_couplings();

   observables.AMU = NSM_a_muon::calculate_a_muon(MODEL);
