...
// File generated at Sun 24 Sep 2017 15:56:28
...
#include "NSM_two_scale_spectrum_generator.hpp"
#endif
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNSMMessage", 1);
...
public:
   ...
   virtual void fill_slha_io(NSM_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const NSM_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NSM_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<NSM_slha<NSM<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   NSM_scales scales{};              ///< scale information
   NSM_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const NSM_input_parameters& input_) { input = input_; }
...
private:
   NSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<NSM_spectrum> spectrum{nullptr};  ///< spectrum information

   NSM_slha_io get_slha_io() const;
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(Lambda1Input), "Lambda1Input");
   MLPutRuleTo(link, INPUTPARAMETER(Lambda2Input), "Lambda2Input");
   MLPutRuleTo(link, INPUTPARAMETER(Lambda3Input), "Lambda3Input");
   MLPutRuleTo(link, INPUTPARAMETER(Lambda4Input), "Lambda4Input");
   MLPutRuleTo(link, INPUTPARAMETER(Lambda5Input), "Lambda5Input");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, NSM_info::particle_names[i].c_str());
      }
   }
}
...
NSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   NSM_slha_io slha_io;
...
void put_spectrum(const NSM_slha<Model>& model, MLINK link)
{
   MLPutRule(link, NSM_info::model_name);
   MLPutFunction(link, "List", 54);
   ...
   MLPutRuleTo(link, MODELPARAMETER(g2), "g2");
   MLPutRuleTo(link, MODELPARAMETER(g3), "g3");
   MLPutRuleTo(link, MODELPARAMETER(Lambda2), "\u03bb\u0032");
   MLPutRuleTo(link, MODELPARAMETER(Lambda3), "\u03bb\u0033");
   MLPutRuleTo(link, MODELPARAMETER(Lambda1), "\u03bb\u0031");
   MLPutRuleTo(link, MODELPARAMETER(Yu), "Yu");
   MLPutRuleTo(link, MODELPARAMETER(Yd), "Yd");
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(Lambda5), "\u03bb\u0035");
   MLPutRuleTo(link, MODELPARAMETER(Lambda4), "\u03bb\u0034");
   MLPutRuleTo(link, MODELPARAMETER(mS2), "mS2");
   MLPutRuleTo(link, MODELPARAMETER(mH2), "mH2");
   MLPutRuleTo(link, MODELPARAMETER(vH), "vH");
...
template <typename Solver_type>
void NSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const NSM_input_parameters& input
)
{
   NSM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void NSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void NSM_spectrum_impl<Solver_type>::fill_slha_io(NSM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   NSM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NSM_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(Lambda1Input) = pars[c++];
   INPUTPARAMETER(Lambda2Input) = pars[c++];
   INPUTPARAMETER(Lambda3Input) = pars[c++];
   INPUTPARAMETER(Lambda4Input) = pars[c++];
   INPUTPARAMETER(Lambda5Input) = pars[c++];
...
DLLEXPORT int FSNSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMGetSettings"))
...
DLLEXPORT int FSNSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::NSM_librarylink;
...
DLLEXPORT int FSNSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNSMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
