...
*/
    ...
    private:
      ...
      // Get the 1D index from the pair (idx1, idx2).
      // Assume row-major order (as done in C and C++)
      size_t get_idx(size_t idx1, size_t idx2) const
      {
        if ((idx1 >= sizeX_) || (idx2 >= sizeY_) || (idx1 < 0) || (idx2 < 0))
        {
          throw std::out_of_range("Index of vector_2D is out of range.");
        }
        return idx1 * sizeX_ + idx2;
      }

    public:
      // Do not allow to create an empty instance
      vector_2D() = delete;

      // Only allowed constructor is to state the size in each direction
      // This is a workaround which makes it possible to specify th dimension at runtime.
      vector_2D(size_t xs, size_t ys):
      sizeX_(xs),
      sizeY_(ys),
      data_( std::unique_ptr<T[]>( new T[xs*ys]() ) )
      {
        //std::cout << "Created instance of 'vector_2D': ";
        //std::cout << "(this= "<<this<<", sizeX_="<<sizeX_<<", sizeY_="<<sizeY_<<")";
        //std::cout << std::endl;
      };

      // Explicitely ask the compiler to get a default destructor.
      // (This usually happens anyway)
      ~vector_2D() = default;

      // Do not allow to move or to copy the object
      // (at least via the default implementations)
      // it will mess with the unique pointer.
      // (One could carfully implement this, but not now)
      vector_2D(const vector_2D&) = delete;
      vector_2D(vector_2D&&) = delete;
      vector_2D& operator=(const vector_2D&) = delete;
      vector_2D& operator=(vector_2D&&) = delete;

      // Read/Write access to entry with multiindex [idx1,idx2]
      T& operator()(size_t i1, size_t i2)
      {
        size_t contidx = get_idx(i1, i2);
        return *( data_.get() + contidx);
      }

      // Read-only access to entry with multiindex [idx1,idx2]
      const T& operator()(size_t i1, size_t i2) const
      {
        size_t contidx = get_idx(i1, i2);
        return T(*( data_.get() + contidx));
      }
