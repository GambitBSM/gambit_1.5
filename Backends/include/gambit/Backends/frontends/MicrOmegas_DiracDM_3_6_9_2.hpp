...
BE_ALLOW_MODELS(DiracDM, StandardModel_Higgs)
...
BE_INI_FUNCTION
{
     ...
     *ForceUG=1;
     ...
     // Set DiracDM model parameters in micrOmegas
     error = assignVal((char*)"mF", *Param["mF"]);
     if (error != 0) BackendIniBit_error().raise(LOCAL_INFO, "Unable to set mF in"
             "MicrOmegas. MicrOmegas error code: " + std::to_string(error));     

     error = assignVal((char*)"lF", *Param["lF"]);
     if (error != 0) BackendIniBit_error().raise(LOCAL_INFO, "Unable to set lF in"
             "MicrOmegas. MicrOmegas error code: " + std::to_string(error));     
