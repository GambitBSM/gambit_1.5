...
BE_NAMESPACE
{
  ...
  DarkAges::fz_table gather_results()
  {
    ...
    logger().send("Message from 'gather_results' backend convenience function in DarkAges v1.1.0 wrapper (Start)",LogTags::info);
    ...
    logger().send("Message from 'gather_results' backend convenience function in DarkAges v1.1.0 wrapper (Done casting NumPy-arrays into std vectors)",LogTags::info);
