...
///  \file
///
///  Frontend for the exoclass 2.7.0 backend
///
///  *********************************************
///
...
#include "gambit/Backends/frontends/exoclass_2_7_0.hpp"
...
BE_NAMESPACE
{
  ...
  ExoClass::file_content fc;
  ExoClass::precision pr;
  ExoClass::background ba;
  ExoClass::thermo th;
  ExoClass::perturbs pt;
  ExoClass::transfers tr;
  ExoClass::primordial pm;
  ExoClass::spectra sp;
  ExoClass::nonlinear nl;
  ExoClass::lensing le;
  ExoClass::output op;
  ExoClass::ErrorMsg errmsg;
  ...
  void exoclass_2_7_0_run(bool has_energy_injection)
  {
    //std::cout << "Last seen alive in: exoclass_2_7_0_run" << std::endl;
    ...
    if (has_energy_injection)
    {
      ...
      th.annihil_coef_xe = (double *) malloc(num_lines*sizeof(double));
      th.annihil_coef_heat = (double *) malloc(num_lines*sizeof(double));
      th.annihil_coef_lya = (double *) malloc(num_lines*sizeof(double));
      th.annihil_coef_ionH = (double *) malloc(num_lines*sizeof(double));
      th.annihil_coef_ionHe = (double *) malloc(num_lines*sizeof(double));
      th.annihil_coef_lowE = (double *) malloc(num_lines*sizeof(double));
      ...
      {
        th.annihil_coef_xe[it] = z.at(it);
        th.annihil_coef_heat[it] = fh.at(it);
        th.annihil_coef_lya[it] = fly.at(it);
        th.annihil_coef_ionH[it] = fhi.at(it);
        th.annihil_coef_ionHe[it] = fhei.at(it);
        th.annihil_coef_lowE[it] = flo.at(it);
      }
  ...
  CosmoBit::Class_container get_exoclass_2_7_0()
  {
    return cosmo;
  }

  void exoclass_2_7_0_set_parameter(CosmoBit::ClassInput in_dict)
  {
    ...
    for(auto iter=in_map.begin(); iter != in_map.end(); iter++)
    {
      ...
      std::strncpy(fc.name[pos], iter->first.c_str(), sizeof(ExoClass::FileArg));
      std::strncpy(fc.value[pos], iter->second.c_str(), sizeof(ExoClass::FileArg));
    }
  }
...
BE_INI_FUNCTION
{
  ...
  exoclass_2_7_0_set_parameter(in_dict);
  in_dict.clear();
  if (has_energy_injection)
