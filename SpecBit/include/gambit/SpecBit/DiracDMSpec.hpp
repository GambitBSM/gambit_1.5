...
///  \file
///
///  DiracDM derived version of SubSpectrum class
///
///  *********************************************
///
///  Authors: 
...
#ifndef DIRACDMSPEC_H
#define DIRACDMSPEC_H
...
#include "gambit/SpecBit/DiracDMSpec_head.hpp"   // "Header" declarations for DiracDMSpec class
...
namespace Gambit
{
   ...
   {
      template <class MI>
      const int DiracDMSpec<MI>::_index_offset = MI::index_offset;

      template <class MI>
      DiracDMSpec<MI>::DiracDMSpec(MI mi, str be_name, str be_version)
      ...
      template <class MI>
      void DiracDMSpec<MI>::RunToScaleOverride(double scale)
      {
        model_interface.model.run_to(scale);
      }
      template <class MI>
      double DiracDMSpec<MI>::GetScale() const
      {
        return model_interface.model.get_scale();
      }
      template <class MI>
      void DiracDMSpec<MI>::SetScale(double scale)
      {
        model_interface.model.set_scale(scale);
      }
      
      template <class MI>
      std::string DiracDMSpec<MI>::AccessError(std::string state) const
      {
        std::string errormsg;
      ...
      template <class MI>
      typename DiracDMSpec<MI>::GetterMaps DiracDMSpec<MI>::fill_getter_maps()
      {
         typename DiracDMSpec<MI>::GetterMaps map_collection;
...

        {
...

         {
            ...
            tmp_map["lF"]= &Model::get_lF;  // ??? not sure why it wants it here
         ...
         {  
            ...
            tmp_map["F"] =  &Model::get_Mxx_pole_slha;
            tmp_map["Dirac"] =  &Model::get_Mxx_pole_slha; // alternative naming convention as in DiracDM container
      ...
      template <class MI>
      typename DiracDMSpec<MI>::SetterMaps DiracDMSpec<MI>::fill_setter_maps()
      {
         typename DiracDMSpec<MI>::SetterMaps map_collection;
         typedef typename MI::Model Model;
