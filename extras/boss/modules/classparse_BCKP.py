...
def run():
    ...
    for class_name_long, class_el in cfg.class_dict.items():
        ...
        master_header_include = '#include "' + wrapper_header_fname + '"\n'
...
def generateWrapperHeader(class_el, class_name, abstr_class_name, namespaces, 
    ...
    # 
    # The class declaration goes into a header with a '_decl' suffix
    # while all member function implementations go into a header with a 
    # '_def' suffix.
    #
    ...
    # decl_code += 2*indent + 'bool member_variable;\n'
    ...
    # decl_code += 2*indent + abstr_class_name['long'] + '* BEptr;\n'
        ...
        for remove_n_args in range(n_overloads+1):
            ...
            if return_is_loaded: 
                if is_ref:
                    ...
                    decl_code += 'reference_returner< ' + w_return_type_for_templ + ', ' + abs_return_type_for_templ +  ' >( BEptr->' + call_func_name + args_bracket_notypes + ' );\n'
                elif (not is_ref) and (pointerness > 0):
                    abs_return_type_for_templ = classutils.toAbstractType(return_type, include_namespace=True, remove_reference=True, remove_pointers=True)
                    w_return_type_for_templ = classutils.toWrapperType(return_type, remove_reference=True, remove_pointers=True)
                    decl_code += 'pointer_returner< ' + w_return_type_for_templ + ', ' + abs_return_type_for_templ +  ' >( BEptr->' + call_func_name + args_bracket_notypes + ' );\n'
                else:
                    decl_code += use_return_type + '( BEptr->' + call_func_name + args_bracket_notypes + ' );\n'
            else:                
                decl_code += 'BEptr->' + call_func_name + args_bracket_notypes + ';\n'
    ...
    for var_el in class_variables:
        ...
        pointerness, is_ref = utils.pointerAndRefCheck(var_el)
        var_type, var_type_kw, var_type_id = utils.findType(var_el)
        wrapper_type_name = classutils.toWrapperType(var_type, remove_reference=True, remove_pointers=True)

        if utils.isLoadedClass(var_type, byname=True):
            if pointerness == 0:
                common_init_list_code += 3*indent + var_name + '(&(BEptr->' + var_name + '_ref' + cfg.code_suffix + '())),\n'
            elif pointerness == 1:
                common_init_list_code += 3*indent + var_name + '(new ' + wrapper_type_name + '(BEptr->' + var_name + '_ref' + cfg.code_suffix + '())),\n'
            else:
    ...
    # decl_code += 3*indent + 'BEptr(in),\n'
    decl_code += 3*indent + wrapper_base_class_name + '( in, memvar_in )'  # FIXME: This is not general. Fix argument list.
    if common_init_list_code != '':
        decl_code += ',\n' + common_init_list_code
    else:
        decl_code += '\n'
    decl_code += common_constructor_body
    ...
    if has_copy_constructor:
        ...
        # decl_code += 3*indent + 'BEptr(in),\n'
        decl_code += 3*indent + wrapper_base_class_name + '(in)'  # FIXME: This is not general. Fix argument list.
        if common_init_list_code != '':
            decl_code += ',\n' + common_init_list_code
        else:
            decl_code += '\n'
    ...
    include_statements.append( '#include "' + cfg.frwd_decls_wrp_fname + cfg.header_extension + '"')
    ...
    include_statements = list( set(include_statements) )
    include_statements_code = '\n'.join(include_statements) + 2*'\n'
    decl_code = include_statements_code + decl_code
