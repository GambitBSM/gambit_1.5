...
void Sigma_MC4BSM_2012_UFO_gg_uvuvx::initProc() 
{
...
void Sigma_MC4BSM_2012_UFO_gg_uvuvx::sigmaKin() 
{
...
double Sigma_MC4BSM_2012_UFO_gg_uvuvx::sigmaHat() 
{
...
void Sigma_MC4BSM_2012_UFO_gg_uvuvx::setIdColAcol() 
{
  if(id1 == 21 && id2 == 21)
  {
  ...
  // Pick color flow
  int ncolor[1] = {2}; 
  if((id1 == 21 && id2 == 21 && id3 == 9000008 && id4 == -9000008))
  {
    vector<double> probs; 
...
double Sigma_MC4BSM_2012_UFO_gg_uvuvx::weightDecay(Event& process, int iResBeg,
    int iResEnd)
{
...
void Sigma_MC4BSM_2012_UFO_gg_uvuvx::calculate_wavefunctions(const int perm[],
    const int hel[])
{
  ...
  VVV1P0_1(w[0], w[1], pars->GC_9, pars->ZERO, pars->ZERO, w[4]); 
  FFV1_1(w[2], w[0], pars->GC_10, pars->mdl_Muv, pars->mdl_Wuv, w[5]); 
  FFV1_2(w[3], w[0], pars->GC_10, pars->mdl_Muv, pars->mdl_Wuv, w[6]); 

  // Calculate all amplitudes
  // Amplitude(s) for diagram number 0
  FFV1_0(w[3], w[2], w[4], pars->GC_10, amp[0]); 
  FFV1_0(w[3], w[5], w[1], pars->GC_10, amp[1]); 
  FFV1_0(w[6], w[2], w[1], pars->GC_10, amp[2]); 


}
double Sigma_MC4BSM_2012_UFO_gg_uvuvx::matrix_gg_uvuvx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 3; 
