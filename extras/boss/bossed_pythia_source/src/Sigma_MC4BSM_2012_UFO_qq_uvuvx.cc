...
// Process: u u~ > uv uv~ WEIGHTED=2
// Process: c c~ > uv uv~ WEIGHTED=2
// Process: d d~ > uv uv~ WEIGHTED=2
// Process: s s~ > uv uv~ WEIGHTED=2

//--------------------------------------------------------------------------
// Initialize process.

void Sigma_MC4BSM_2012_UFO_qq_uvuvx::initProc() 
{
  ...
  jamp2[1] = new double[2]; 
}

//--------------------------------------------------------------------------
// Evaluate |M|^2, part independent of incoming flavour.

void Sigma_MC4BSM_2012_UFO_qq_uvuvx::sigmaKin() 
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[2] = matrix_uux_uvuvx(); 
        t[3] = matrix_ccx_uvuvx(); 
        double tsum = 0; 
...
double Sigma_MC4BSM_2012_UFO_qq_uvuvx::sigmaHat() 
{
...
void Sigma_MC4BSM_2012_UFO_qq_uvuvx::setIdColAcol() 
{
  if(id1 == -3 && id2 == 3)
  {
    // Pick one of the flavor combinations
    int flavors[1][2] = {{9000008, -9000008}}; 
    vector<double> probs; 
  ...
  {
    // Pick one of the flavor combinations (9000008, -9000008)
    int flavors[1][2] = {{9000008, -9000008}}; 
    vector<double> probs; 
  ...
  // Pick color flow
  int ncolor[2] = {2, 2}; 
  if((id1 == 2 && id2 == -2 && id3 == 9000008 && id4 == -9000008))
  {
    vector<double> probs; 
  ...
  else if((id1 == 4 && id2 == -4 && id3 == 9000008 && id4 == -9000008) || (id1
      == 1 && id2 == -1 && id3 == 9000008 && id4 == -9000008) || (id1 == 3 &&
      id2 == -3 && id3 == 9000008 && id4 == -9000008))
  {
    vector<double> probs; 
    double sum = jamp2[1][0] + jamp2[1][1]; 
  ...
  else if((id1 == -2 && id2 == 2 && id3 == 9000008 && id4 == -9000008))
  {
    vector<double> probs; 
    double sum = jamp2[0][0] + jamp2[0][1]; 
    for(int i = 0; i < ncolor[0]; i++ )
...
double Sigma_MC4BSM_2012_UFO_qq_uvuvx::weightDecay(Event& process, int iResBeg,
    int iResEnd)
{
...
void Sigma_MC4BSM_2012_UFO_qq_uvuvx::calculate_wavefunctions(const int perm[],
    const int hel[])
{
  ...
  FFV1P0_3(w[0], w[1], pars->GC_10, pars->ZERO, pars->ZERO, w[4]); 
  FFS4_3(w[0], w[2], pars->GC_75, pars->mdl_MPe1, pars->mdl_Wpe1, w[5]); 
  FFS4_3(w[0], w[2], pars->GC_77, pars->mdl_MPe2, pars->mdl_Wpe2, w[6]); 

  // Calculate all amplitudes
  // Amplitude(s) for diagram number 0
  FFV1_0(w[3], w[2], w[4], pars->GC_10, amp[0]); 
  FFS3_0(w[3], w[1], w[5], pars->GC_75, amp[1]); 
  FFS3_0(w[3], w[1], w[6], pars->GC_77, amp[2]); 


}
double Sigma_MC4BSM_2012_UFO_qq_uvuvx::matrix_uux_uvuvx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 3; 
...
double Sigma_MC4BSM_2012_UFO_qq_uvuvx::matrix_ccx_uvuvx() 
{
  int i, j; 
  // Local variables
  const int ngraphs = 1; 
