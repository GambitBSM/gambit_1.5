...
int main()
{
  ...
  typedef GAMBIT::Classes::LibFirst::v1_0::Base Base;
  ...
  std::cout << Crusher->returnvalue() << " is the base rate.  Hrrm. ";
  std::cout << "OK, time to cash it in!" << std::endl;
  delete Crusher;

  std::cout << std::endl;
  std::cout << "Creating a proprietary Base object using libFirst..." << std::endl;
  Base* myBase = GAMBIT::Backend::LibFirst::BaseFactory();
  std::cout << "Checking how much I can get for it:" << std::endl;
  std::cout << myBase->valueMe(4.45) << " dollars.  That's pretty lousy. ";
  std::cout << "Oh well, I have no use for a base anyway..." << std::endl;
  delete myBase;
