...
int main(int argc, char* argv[]){
  ...
  /* Initialize any experiments to be used.  The argument indicates if
     ...
     actually necessary here (the flag is ignored for experiments that
     do not have the event energies necessary for a maximum gap
     analysis). */
  XENON100_2012_Init(true);
  LUX_2013_Init(true);
  SuperCDMS_2014_Init(true);
  SIMPLE_2014_Init(true);
