...
! the parameters (dslnpin is fairly robust in this respect); however,
...
!=======================================================================
! 
      REAL*8 FUNCTION dslnpiln(n,thetab,thetas,sigma)
      IMPLICIT NONE
      INTEGER n
     ...
     &    .OR. (thetas .LT. 0d0) .OR. (sigma .LT. 0d0)) THEN
        WRITE(*,*) 'ERROR: dslnpiln called with negative argument'
        STOP
      END IF
      ...
      IF (sigma .EQ. 0d0) THEN
        dslnpiln = lnpoisson(n,thetab+thetas)
        RETURN
      END IF
      
      ! General case ---------------------------------------
      ! For the general case, we will do a numerical integration.
      dslnpiln = nintegrate(n,thetab,thetas,sigma)
