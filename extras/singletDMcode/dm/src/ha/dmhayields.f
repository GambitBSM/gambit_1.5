*****************************************************************************
*** function dshayields calculates the yield above threshold (yieldk=1) or the
...
*****************************************************************************
      ...
      real*8 function dmhayields(eh,emuth,hno,yieldk,istat)
...
c------------------------ functions ------------------------------------
      ...
      real*8 dmhayields2,dmhaemean,dmhayieldfth,dmhayielddec
...
c      write(*,*) 'dshayields called with:'
c      write(*,*) '     eh = ',eh
c      write(*,*) '  emuth = ',emuth
c      write(*,*) '    hno = ',hno
...
c..."complex" channels
     ...
     &      3,yieldk,istat)
        ...
        if (has0br(7,hno).gt.0.0d0) then    ! S+ S- channel
          ...
          yield=yield+2.0*
     &      has0br(7,hno)*dmhayields2(e1,emuth,4,yieldk,istat)
        endif
     ...
     &       has0m(1),emuth,6,yieldk,istat)
          yield=yield+has0br(8,hno)*dmhayields2(e2,emuth,1,yieldk,istat)
        endif
     ...
     &       has0m(2),emuth,6,yieldk,istat)
          yield=yield+has0br(9,hno)*dmhayields2(e2,emuth,2,yieldk,istat)
        endif
     ...
     &       has0m(3),emuth,6,yieldk,istat)
          yield=yield
     &      +has0br(10,hno)*dmhayields2(e2,emuth,3,yieldk,istat)
        endif
     ...
     +    dmhayieldfth(eh,has0m(hno),map(5),hascm,emuth,5,yieldk,istat)
          yield=yield
     &        +has0br(11,hno)*dmhayields2(e2,emuth,4,yieldk,istat)
        endif
