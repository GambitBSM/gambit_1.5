...
//  double psquare = ((-36.61253*-36.61253) + (-38.7890*-38.7890) + (-27.47610)*(-27.47610));
//  double energy = sqrt(psquare - (0.0511e-03)*(0.0511e-03));

//  hep_simple_lib::Particle *chosen = new hep_simple_lib::Particle(-36.61253,-38.7890,-27.47610,energy,11);
//             _electrons.push_back(chosen);
...
void MyAnalysis::PrintParticles( vector<hep_simple_lib::Particle*> list)
{
  for (int j=0;j<(int)list.size();j++) {
    cout << "Particle " << j << " "<< list[j]->pid() << " P: "<< list[j]->mom().px() << " " << list[j]->mom().py() << " " << list[j]->mom().pz() << " " << list[j]->mom().E()
      << " Eta: " << list[j]->mom().eta() << " Phi: " << list[j]->mom().phi()  << " Pt: " << list[j]->pT() << " Mass: " << list[j]->mom().m()  << endl;
  }
                   ...
                   int &debug_level) 
{
  ...
  // the particle gun pdgid
  if (cmdOptionExists(argv, argv+argc, "-pid")) 
     particle_pdgid = atoi(getCmdOption(argv, argv + argc, "-pid").c_str());
...
//  gInterpreter->EnableAutoLoading();
  ...
  cout << " PdgId for particle gun is " << particle_gun_pdgid;
