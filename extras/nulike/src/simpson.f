...
!
!  Input:  integrand f, auxilary function aux, context ptr
...
!=======================================================================
      ...
      nulike_simpson=0.d0
      del=b-a
      ost=0.5*del*(f(a,aux,ptr)+f(b,aux,ptr))
      x=0.5*(b+a)
      st=0.5*(ost+del*f(x,aux,ptr))
      ost=st
      it=1
      nfcn=3
      do j=3,jmax
