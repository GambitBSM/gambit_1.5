...
***********************************************************************
      ...
      real*8 y, Elep, log10Elep, mlep2, phi_obs, phi_err, errlep, plep, 
     &philep_p, philep_n
      real*8 spnc, spcc, snnc, sncc, pcont, ncont, edisp, effvol, anglos
     &s, dsdxdy 
      real*8 nulike_edisp, nulike_psf, nulike_sens, nulike_angres, cosan
     &g
      integer ptype
      external dsdxdy
      ...
      Elep = Eshare * (1.d0 - y)                                        
     &       ! GeV
      if (Elep .le. 0.d0) then                                          
     &       ! Abort if the lepton energy is zero.
     ...
     &            
      ...
      !Effective volume
      effvol = nulike_sens(log10Elep, ptypeshare)                       
     &       ! km^3
      if (effvol .le. 0.d0) then                                        
     &       ! Abort if effective volume is
        nulike_partintegrand2 = 0.d0                                    
     &       ! zero.      
        return
      endif
      ...
      if (eventnumshare .ne. 0) then
        edisp = nulike_edisp(log10Elep, events_ee(eventnumshare, analysi
     &s), 2014)! [ee]^-1
        if (edisp .le. epsilon(effvol)) then                            
     &         ! Abort if energy dispersion is
          nulike_partintegrand2 = 0.d0                                  
     &         ! zero.     
          return
        endif
      ...
      !Angular loss factor
      errlep = nulike_angres(log10Elep)                                 
     &       ! degrees
      angloss = 1.d0-exp(-0.5d0*phi_max*phi_max/(errlep*errlep))        
     &       ! dimensionless

      !Cross-sections and densities 
      ptype = ptypeshare + 2*(leptypeshare-1)                           
     &       ! Convert to nusigma internal neutrino number.
      spnc = dsdxdy(Eshare,xshare,y,ptype,'p','NC')                     
     &       ! cm^2
      spcc = dsdxdy(Eshare,xshare,y,ptype,'p','CC')                     
     &       ! cm^2
      snnc = dsdxdy(Eshare,xshare,y,ptype,'n','NC')                     
     &       ! cm^2
      sncc = dsdxdy(Eshare,xshare,y,ptype,'n','CC')                     
     &       ! cm^2
      pcont = numdens_p * (spnc + spcc)                                 
     &       ! 1e-5 m^-3 cm^2 
      ncont = numdens_n * (snnc + sncc)                                 
     &       ! 1e-5 m^-3 cm^2

      !Point-spread functions
      if (eventnumshare .ne. 0) then

        mlep2 = lepmass(leptypeshare)*lepmass(leptypeshare)             
     &       ! GeV^2
        plep = dsqrt(Elep*Elep - mlep2)                                 
     &       !GeV
        phi_obs = acos(events_cosphi(eventnumshare,analysis)) * 180.d0/p
     &i      ! --> degrees
        phi_err = events_cosphiErr(eventnumshare,analysis)              
     &       ! Already in degrees.
      ...
      if (eventnumshare .ne. 0) nulike_partintegrand2 = nulike_partinteg
     &rand2*edisp  ! -->m^2 deg^-1 [ee]^-1
      ...
      if (nulike_partintegrand2 .ne. nulike_partintegrand2) then
        ...
        if (eventnumshare .ne. 0) then
          ...
          write(*,*) nulike_psf(phi_obs, philep_n, phi_err), nulike_psf(
     &phi_obs, philep_p, phi_err)
        endif
