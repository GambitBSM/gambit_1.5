!=======================================================================
! LN(Poisson Integral) for Normal systematic uncertainty (SUMs)
! 
...
! systematic uncertainty is taken to be a normally distributed
...
!       EXP(lnlesum) + EXP(lngesum) = 1 + EXP(lnpin)
! 
...
!=======================================================================
! 
      SUBROUTINE nulike_lnpinsum(n,thetab,thetas,sigma,
     &                      lnpin,lnlesum,lngesum)
      IMPLICIT NONE
      INTEGER n
      REAL*8 thetab,thetas,sigma,lnpin,lnlesum,lngesum
     ...
     &    .OR. (thetas .LT. 0d0) .OR. (sigma .LT. 0d0)) THEN
        print*, 'ERROR: nulike_lnpinsum called with negative argument'
        STOP
      END IF
      ...
      IF (thetas .EQ. 0d0) THEN
        CALL poissonsums(n,thetab,lnpin,lnlesum,lngesum)
        RETURN
      END IF
      ...
      IF (sigma .EQ. 0d0) THEN
        CALL poissonsums(n,thetab+thetas,lnpin,lnlesum,lngesum)
        RETURN
      END IF
      
      ! General case ---------------------------------------
      CALL numericalsums(n,thetab,thetas,sigma,lnpin,lnlesum,lngesum)
     ...
     &                         lnpmf,lnlesum,lngesum)
      ...
      INTEGER k
      REAL*8 lnp,nulike_lnpin
      LOGICAL is_increasing
      
      ! Special case: n = 0
      IF (n .EQ. 0) THEN
        lnpmf   = nulike_lnpin(n,thetab,thetas,sigma)
        lnlesum = lnpmf
        lngesum = 0d0
        RETURN
      END IF
      ...
      lnp = nulike_lnpin(n+1,thetab,thetas,sigma)
      ...
      IF (is_increasing) THEN
        ...
        DO WHILE (k .GT. 0)
          k = k-1
          lnp = nulike_lnpin(k,thetab,thetas,sigma)
          lnlesum = lnsum(lnlesum,lnp)
          IF (lnp - lnlesum .LE. LN_PRECISION) EXIT
        END DO
        lngesum = LOG(1d0 - (EXP(lnlesum) - EXP(lnpmf)))
