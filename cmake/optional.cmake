...
if (WITH_ROOT AND ROOT_FOUND)
  ...
  set(CXX14 "c\\+\\+14")
  set(CXX11 "c\\+\\+11")
  string(REGEX MATCH ${CXX17} ROOT_USES_CXX17 ${ROOT_CXX_FLAGS})
  string(REGEX MATCH ${CXX14} ROOT_USES_CXX14 ${ROOT_CXX_FLAGS})
  string(REGEX MATCH ${CXX11} ROOT_USES_CXX11 ${ROOT_CXX_FLAGS})
  if (ROOT_USES_CXX17)
    message("${BoldYellow}   This ROOT was compiled with -std=c++17.${ColourReset}")
    if (NOT COMPILER_SUPPORTS_CXX17)
      message(FATAL_ERROR "${BoldRed}This installation of ROOT has been compiled with C++17 support, "
                          "but your chosen compiler does not support C++17.  Please change compiler "
                          "or set -DWITH_ROOT=OFF.${ColourReset}")
    endif()
  elseif (ROOT_USES_CXX14)
    message("${BoldYellow}   This ROOT was compiled with -std=c++14.${ColourReset}")
    if (COMPILER_SUPPORTS_CXX17)
      unset(COMPILER_SUPPORTS_CXX17)
      CHECK_CXX_COMPILER_FLAG("-std=c++14" COMPILER_SUPPORTS_CXX14)
      if(COMPILER_SUPPORTS_CXX14)
        string(REGEX REPLACE ${CXX17} "-std=c++14" CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
        string(REGEX REPLACE ${CXX17} "-std=c++14" BACKEND_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
      endif()
    endif()
    if (NOT COMPILER_SUPPORTS_CXX14)
      message(FATAL_ERROR "${BoldRed}This installation of ROOT has been compiled with C++14 support, "
                          "but your chosen compiler does not support C++14.  Please change compiler "
                          "or set -DWITH_ROOT=OFF.${ColourReset}")
    endif()
  elseif (ROOT_USES_CXX11)
    message("${BoldYellow}   This ROOT was compiled with -std=c++11.${ColourReset}")
    if (COMPILER_SUPPORTS_CXX17 OR COMPILER_SUPPORTS_CXX14)
      unset(COMPILER_SUPPORTS_CXX14)
      unset(COMPILER_SUPPORTS_CXX17)
      CHECK_CXX_COMPILER_FLAG("-std=c++11" COMPILER_SUPPORTS_CXX11)
      if(COMPILER_SUPPORTS_CXX11)
        string(REGEX REPLACE ${CXX17} "-std=c++11" CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
        string(REGEX REPLACE ${CXX17} "-std=c++11" BACKEND_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
        string(REGEX REPLACE ${CXX14} "-std=c++11" CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
        string(REGEX REPLACE ${CXX14} "-std=c++11" BACKEND_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
      endif()
    endif()
    if (NOT COMPILER_SUPPORTS_CXX11)
      message(FATAL_ERROR "${BoldRed}This installation of ROOT has been compiled with C++11 support, "
                          "but your chosen compiler does not support C++11.  Please change compiler "
                          "or set -DWITH_ROOT=OFF.${ColourReset}")
    endif()
  else()
