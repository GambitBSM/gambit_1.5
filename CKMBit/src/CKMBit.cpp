  ...
  namespace CKMBit
  {
    ...
    void nevents_pred_rounded(int &result)
    {
      ...
      if (Random::draw() < x)
      {
        ...
        //CKMBit_error().raise(LOCAL_INFO,"Damn, this integer event count is bad.");
    ...
    void function_pointer_retriever( double(*&result)(int&) )
    {
      ...
      else CKMBit_error().raise(LOCAL_INFO,"Unrecognised choice from external_funcs BEgroup.");
    }
    ...
    void example_damu (double &result)
    {
      ...
      std::cout << "In CKMBit, function damu" << std::endl;
    ...
    void lnL_gaussian (double &result)
    {
      ...
      if (ModelInUse("NormalDist"))
      {
        for (int i=0; i<N; ++i)
        {
          ...
          {
            CKMBit_error().raise(LOCAL_INFO,"NaN detected in input parameters for model"
      ...
      {
         CKMBit_error().raise(LOCAL_INFO,"Whoops, you are not scanning the model "
          " NormalDist! There is probably a bug CKMBit_rollcall.hpp; this module "
    ...
    void eventLoopManager()
    {
      ...
      piped_warnings.check(CKMBit_warning());
      piped_errors.check(CKMBit_error());
    ...
    void exampleEventGen(float &result)
    {
      ...
      //   //   CKMBit_error().raise(LOCAL_INFO,"Error triggered for testing purposes.");
    ...
    void marg_poisson_test(double &result)
    {
        ...
        else CKMBit_error().raise(LOCAL_INFO,"Unrecognised choice from lnlike_marg_poisson BEgroup.");
