...
///  \file
///
///  Example of GAMBIT CKMBit standalone
///  main program.
///
///  *********************************************
///
...
#include "gambit/CKMBit/CKMBit_rollcall.hpp"

// Only needed here
#include "gambit/Utils/util_functions.hpp"

using namespace CKMBit::Accessors;    // Helper functions that provide some info about the module
using namespace CKMBit::Functown;     // Functors wrapping the module's actual module functions
using namespace BackendIniBit::Functown;    // Functors wrapping the backend initialisation functions

// Register ad-hoc module functions for use; as many or as few models can be given as desired.
// Full declaration as per regular rollcall headers such as CKMBit_rollcall.hpp is also allowed.
//QUICK_FUNCTION(CKMBit, xsection, NEW_CAPABILITY, local_xsection, double, (NUHM1))
QUICK_FUNCTION(CKMBit, xsection, OLD_CAPABILITY, local_xsection, double, (NUHM1))

// Ad-hoc functions for filling dependencies that cannot or should not otherwise be filled from inside CKMBit.
namespace Gambit
{
  namespace CKMBit
  {
...
int main()
{
  ...
  try
  {
    ...
    std::cout << std::endl << "Starting CKMBit standalone example" << std::endl;
    std::cout << "----------" << std::endl;

    //Initialise logging (just comment out if you want no logfiles)
    initialise_standalone_logs("runs/CKMBit_standalone/logs/");

    // Change the fatality of different errors and warnings from the defaults, if desired.
    model_warning().set_fatal(true);
    CKMBit::CKMBit_error().set_fatal(true);

    // Initialise the random number generator.
    Random::create_rng_engine("default");

    // Test message (note: we are not actually "inside" CKMBit here, so the log message will not receive an 'CKMBit' tag).
    logger()<<"Running CKMBit standalone example"<<LogTags::info<<EOM;
    ...
    // Print some example diagnostics about CKMBit
    ...
    std::cout << CKMBit::Pipes::function_pointer_retriever::BEreq::externalFunction.origin() << "::";
    std::cout << CKMBit::Pipes::function_pointer_retriever::BEreq::externalFunction.name() << std::endl;
    ...
    std::cout << CKMBit::Pipes::nevents_pred_rounded::Dep::nevents.origin() << "::";
    std::cout << CKMBit::Pipes::nevents_pred_rounded::Dep::nevents.name() << std::endl;
    std::cout << std::endl << "My function nevents_pred has had its dependency on xsection filled by:" << endl;
    std::cout << CKMBit::Pipes::nevents_pred::Dep::xsection.origin() << "::";
    std::cout << CKMBit::Pipes::nevents_pred::Dep::xsection.name() << std::endl;
    std::cout << std::endl << "My function local_xsection has had its dependency on MSSM parameters filled by:" << std::endl;
    std::cout << CKMBit::Pipes::local_xsection::Dep::NUHM1_parameters.origin() << "::";
    std::cout << CKMBit::Pipes::local_xsection::Dep::NUHM1_parameters.name() << std::endl;
    ...
    std::cout << CKMBit::Pipes::exampleCut::Dep::event.origin() << "::";
    std::cout << CKMBit::Pipes::exampleCut::Dep::event.name() << std::endl;
    ...
    std::cout << "CKMBit standalone example has finished successfully." << std::endl << std::endl;

  }

  catch (std::exception& e)
  {
    std::cout << "CKMBit standalone example has exited with fatal exception: " << e.what() << std::endl;
  }
